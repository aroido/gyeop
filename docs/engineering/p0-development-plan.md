...
비공개 재미 검증에서 active owner 경로는 `/`, `/play/[slug]`, `/me`와 `POST /api/plays`, `GET /api/plays/[playId]`, `PUT /api/plays/[playId]/answers/[cardId]`, `POST /api/plays/[playId]/complete`, `DELETE /api/me/session`이다. 아래 `/auth/*`, account-delete, email/Cron 경로는 production beta candidate이며 현재 검증 gate에는 포함하지 않는다.
...
- owner cookie는 마지막 성공 요청부터 DB 서버 기준 7일, response session cookie는 생성 후 24시간 TTL이며 둘 다 `HttpOnly`, `Secure`, `SameSite=Lax`, `Path=/`다. cookie 만료는 DB가 만든 `management_expires_at`·`session_expires_at`보다 늦을 수 없다.
