...
cookie는 `HttpOnly`, `Secure`, `SameSite=Lax`, `Path=/`, `Domain` 없음이며 성공한 read/save/complete/resume마다 anonymous owner의 DB server time 기준 7일 inactivity window를 갱신한다. 완료 뒤 same-browser PKCE callback이 owner의 모든 play를 Auth UID에 연결한다. 같은 계정의 다른 브라우저는 연결된 완료 play를 다시 조회·관리할 수 있지만 capability secret을 재발급받지는 않는다.
...
| `GET /api/plays/[playId]` | anonymous-owner cookie | cookie owner에 속한 play의 셀프 선택·진행·완료 상태 복구 |
| `PUT /api/plays/[playId]/answers/[cardId]` | anonymous-owner cookie | draft 셀프 답변 idempotent 저장 |
| `POST /api/plays/[playId]/complete` | anonymous-owner cookie | 정확히 10장 검사 후 `draft→completed`; incomplete·retry는 결정적 결과 |
