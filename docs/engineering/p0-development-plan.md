...
| production beta 재승인 | inactive candidate | 이메일 매직 링크 계정 연결, 알림, 계정 삭제·미귀속 Auth cleanup, cross-device 정책 | 전체 연령용 질문팩 검수, 현행 법률 서면 검토, 보관·backup·SMTP·운영 책임 재승인 뒤 별도 활성화 |
...
- `pack_plays(management_expires_at) WHERE owner_id IS NULL AND management_secret_hash IS NOT NULL`
