...
| `analytics_events` | `id`, `event_name`, nullable `visitor_response_id`, 제한된 `properties`, `occurred_at`; 신규 관계·response-start event만 DB-derived response subject를 연결하고 응답 선택값·이메일·원문 토큰 금지. 철회 transaction은 이 ID로 연결 event의 관계·시점 property와 subject ID를 제거 |
