...
|---|---|---|---|
| 비공개 재미 검증 | active | anonymous owner capability, 같은 브라우저 다중 팩·7일 inactivity 복구, 완료 뒤 Google OAuth claim, 인증 계정의 완료 play 조회 | 주인 응답·저장·공유, 방문자 비교·새 주인 전환, 누적 프로필 재공유의 재미 |
| production beta 재승인 | inactive candidate | 공개 가입 운영, 알림, 계정 삭제·미귀속 Auth cleanup | 전체 연령용 질문팩 검수, 현행 법률 서면 검토, 보관·backup·운영 책임 재승인 뒤 별도 활성화 |

이 문서의 익명 owner→Auth claim과 인증 play 조회는 첫 번째 행의 active 계약이다. notification, account deletion/Cron, 미귀속 Auth cleanup과 public signup 운영 상세는 두 번째 행의 후보 설계이며, active 구간과 충돌할 때는 이 절과 `docs/product/core-feature-priority.md`의 공유 직전 claim 계약을 우선한다.

### 2.2 `$0` private MVP 운영 경계

현재 월 인프라 비용 hard cap은 `$0`다. active resource는 이미 연결된 Render Free Web Service 하나, Supabase Free project 하나, public repository GitHub Actions와 local Supabase뿐이며 세부 운영 SSOT는 `docs/engineering/private-mvp-zero-cost-runbook.md`다. 유료 resource, 별도 staging/production, personal Linux host, custom SMTP, Resend delivery, hosted Cron, standalone public signup, self-service/provider Auth deletion과 public production은 비활성·연기한다. 이후 절의 Linux/systemd/Cron/notification/deletion 설계는 future production candidate 참고이며 현재 설치·완료 조건이 아니다.
...
| 데이터·인증 | active: Supabase PostgreSQL, SQL migration, RLS, anonymous-owner capability cookie, Supabase Auth Google OAuth PKCE claim | 익명 시작은 유지하고 공유 직전에만 계정 연결하며, 연결된 완료 play를 인증 계정에서 조회한다. |
| 입력 검증 | Zod | 모든 HTTP 신뢰 경계에서 동일한 스키마를 사용한다. |
| 배포 | active: 기존 Render Free 단일 Docker web service | Render 제공 TLS URL 앞의 HAProxy와 loopback `next start` 한 인스턴스만 사용한다. 별도 staging/production과 Linux host는 inactive candidate다. |
| 알림 이메일 | inactive | `$0` private MVP에는 custom SMTP·Resend delivery를 연결하지 않는다. |
...
위 그림의 Cron·Email 연결은 future production candidate다. 현재 active runtime에는 hosted Cron과 email delivery가 없다.
...
현재 private MVP는 email delivery를 사용하지 않는다. 아래 custom SMTP·Resend 계약은 future production candidate이며, 별도 재승인 전에는 provider를 연결하거나 이메일을 발송하지 않는다.

future production은 Supabase 기본 SMTP를 전달 경로로 사용하지 않는다. 승인된 custom SMTP, 별도 Auth 발신 domain/from, project-wide email 한도와 사용자별 cooldown을 적용한다. Auth SMTP와 notification REST가 같은 Resend team을 쓰면 Supabase Auth 최대 burst와 app-side 매직 링크 rate limit을 포함한 headroom을 notification 승인 RPS에서 제외하고 staging에서 합산 부하를 검증한다. headroom을 증명할 수 없으면 별도 provider team/credential로 격리한다.

아래 목록은 active Render 변수와 inactive production candidate를 함께 기록한 inventory다. 현재 필요한 값은 `render.yaml`이 정하며 `RESEND_*`, notification, Cron credential은 설정하지 않는다.
...
현재 DB migration은 local reset과 CI에서 검증하고 기존 Supabase Free project 적용은 별도 승인된 배포 절차를 따른다. staging·production 순차 적용은 future production candidate다. P0 migration은 additive change를 기본으로 하며, 데이터 삭제·column rename은 별도 migration과 복구 절차 없이는 허용하지 않는다.

## 7. 화면과 URL

비공개 재미 검증에서 active owner 경로는 `/`, `/play/new?pack=<slug>`, `/play/[playId]`, `/auth/sign-in`, `/auth/google`, `/auth/callback`, `/me`, `/me/plays/[playId]`, `/i/[publicId]#k=...`와 owner play·Google OAuth·인증 play 목록·공유·프로필 API다. account-delete, notification/Cron 경로는 production beta candidate이며 현재 검증 gate에는 포함하지 않는다.
...
주인 셀프 draft는 `__Host-gyeop-owner=v1.<anonymousOwnerId>.<32-byte base64url secret>` cookie로 보호한다. 기존 wire format과 `SHA-256("gyeop-owner-play-v1" || 0x00 || raw secret)`은 호환 유지한다. 최초 cookie 없는 `POST /api/plays`는 anonymous owner와 첫 play를 만들고, 유효 cookie는 같은 owner의 요청 팩 play를 재개하거나 팩별 play를 하나 만든다. malformed·tampered·expired·revoked cookie를 새 owner로 조용히 교체하지 않는다.

cookie는 `HttpOnly`, `Secure`, `SameSite=Lax`, `Path=/`, `Domain` 없음이며 성공한 read/save/complete/resume마다 anonymous owner의 DB server time 기준 7일 inactivity window를 갱신한다. 완료 뒤 same-browser PKCE callback이 owner의 모든 play를 Auth UID에 연결한다. 같은 계정의 다른 브라우저는 연결된 draft를 이어 답하고 완료 play를 다시 관리할 수 있지만 capability secret을 재발급받지는 않는다.
...
| `anonymous_owners` | `id`, unique nullable `management_secret_hash`, `last_active_at`, `management_expires_at`, nullable `management_revoked_at`, timestamps; 한 cookie capability가 여러 팩 play를 묶음 |
| `pack_plays` | `id`, `pack_version_id`, `anonymous_owner_id`, nullable `owner_id` FK→`auth.users`, deprecated capability columns, `status` (`draft|completed`), `current_position`, nullable `completed_at`, timestamps; unique `(anonymous_owner_id, pack_version_id)` |
...
| `visitor_responses` | `id`, `share_link_id`, `pack_version_id`, nullable `relationship_code`, nullable `known_since_code`, `status`, unique nullable `session_token_hash`, `last_active_at`, `session_expires_at`, unique nullable `management_token_hash`, `created_at`, `submitted_at`, `withdrawn_at`; draft session은 성공 mutation만 `last_active_at`과 `session_expires_at = last_active_at + 24시간`을 갱신하고 서버 시각으로 판정 |
...
| `auth_registration_states` | `auth_user_id` PK, `created_at`; Auth user 생성 trigger가 넣고 유효한 same-browser answered-draft `claim_play` 또는 grace 종료 unclaimed enqueue transaction만 제거, raw email 금지 |
| `analytics_events` | `id`, `event_name`, nullable `owner_play_id`, `share_link_id`, `visitor_response_id`, 제한된 `properties`, `occurred_at`; subject 조합과 properties는 event별 exact allowlist. 관계·시점·응답 선택값·이메일·원문 토큰 금지. 철회·삭제 시 연결 subject를 제거하고 raw event는 30일 뒤 삭제 |
| `rate_limit_buckets` | `key_hash`, `action`, `window_start`, `count`, `expires_at`; `expires_at + 24시간`부터 삭제 대상으로 전환하고 다음 24시간 안에 삭제 |
...
presentation metadata는 DB schema로 확장하지 않고 사람이 승인한 공식 팩 static catalog에서 관리한다. 추천 관계·민감도는 `pack_templates`, 질문 수는 발행된 카드 10장에서 derive한다. runtime은 미등록 slug, 승인 trace가 없는 variant, 없는 cover asset에서 build를 실패시킨다. 홈은 `default_share_kind`를 최초 추천으로 표시하고 링크 생성 화면도 그 종류를 초기 선택하되, owner가 제출 전에 공개·1:1 중 다른 종류로 바꿀 수 있다. creator/remix/per-card presentation tag는 P1 이후다.

### 8.2 상태

- `pack_plays.status` active 비공개 검증: `draft → completed`; 완료 뒤 self answer는 불변이며 owner claim은 별도 상태 복사 없이 `owner_id` 연결로 표현한다.
...
- `anonymous_owners(management_expires_at) WHERE management_secret_hash IS NOT NULL`
- `pack_plays(anonymous_owner_id, pack_version_id)` unique index
- `pack_plays(owner_id, completed_at desc) WHERE owner_id IS NOT NULL AND status = 'completed'`
...
2. 새 `visitor_responses`와 session token hash를 만들고 DB 서버 시각 기준 `last_active_at = clock_timestamp()`, `session_expires_at = last_active_at + interval '24 hours'`를 저장한다. 관계·시점 확정과 값이 바뀐 draft answer save만 두 시각을 갱신하고, read·동일값 retry·실패 요청은 갱신하지 않는다. cookie `Max-Age`는 DB 시한을 넘지 않게 설정한다.
...
8. P0 response session cookie는 submit 때 회전하지 않는다. submit은 최종 draft mutation으로 `last_active_at = submitted_at`, `session_expires_at = submitted_at + 24시간`을 한 번 기록하고, submitted 상태의 비교·선택 2장 read/write는 더 연장하지 않는다. `save_response_answer`, `submit_response`, `assign_optional_cards`, 방문자용 `get_visitor_response`는 모두 같은 서버 만료 시한을 검사한다. 따라서 commit 뒤 HTTP 응답과 `Set-Cookie`가 유실되어도 아직 만료되지 않은 기존 cookie와 pending management token으로 duplicate submit을 복구할 수 있다.
...
기간·trigger·hard-delete/backup 상한·승인 peak의 canonical SSOT는 `docs/product/data-retention-and-deletion-policy.md`다. 후속 구현은 값을 완화하거나 새 activity clock을 발명하지 않고 이 절의 transaction 경계에 적용한다.

1. public link는 발급 후 30일, 1:1 link는 발급 후 7일 또는 첫 제출에 닫는다. terminal notification job은 `terminal_at`과 `owner_id`·`source_response_id`·keyed `request_fingerprint` 제거를 같은 transaction에서 기록한다. 직접 필드를 지운 뒤에도 최소 참조 tombstone (`id`, `status`, `terminal_at`, `payload_version_id`, `fingerprint_key_version`)과 FK는 `terminal_at + 24시간` 전에 삭제·절단하지 않는다. 그 뒤 24시간 안에 job tombstone을 삭제하고, 다시 24시간 안에 더 이상 보호된 참조가 없는 payload version row만 삭제한다. 같은 drain query가 0이면 운영자가 24시간 안에 fingerprint key reader를 add-reader→switch-writer→drain→retire runbook으로 config에서 제거·재배포한다. DB cleanup RPC는 secret을 변경하지 않는다. 정상 retention cleanup은 참조 notification job이 terminal이 되기 전에 source response를 삭제하거나 `source_response_id`를 비식별화하지 않는다. 철회·owner 삭제만 response 상태 전이, job cancel 요청, 직접 ID/fingerprint 제거를 같은 transaction에서 처리하는 예외다.
2. 단일 `expire_retained_data` 호출의 총 작업 예산은 45초다. 첫 round에서 활성화된 모든 고정 category(후속 `unclaimed_auth`, `auth_deletion_permit_prune`, `auth_deletion_receipt_prune` 포함)를 oldest-due-first bounded chunk 하나씩 반드시 시도하고, 남은 예산만 500 row chunk fair round-robin에 쓴다. category별 query timeout·예외 격리로 한 category 실패가 나머지 첫 round를 막지 않으며 다음 일일 실행 또는 signed catch-up이 미처리 대상을 reconciliation한다. category별 `remaining_count`·`oldest_due_at`, 처리 건수, 오류 code만 기록하고 정책이 정한 물리 cleanup 최대 지연의 50%에서 경보, 시한 위반 예상 시 같은 고정 일일 분기의 signed catch-up을 반복하거나 검토된 schedule 증설을 적용한다. task·table·category·UID 입력은 받지 않는다.
   초기 beta 승인 일일 peak family는 owner/link 1,000, visitor/withdrawn 5,000, raw/aggregate analytics 100,000, rate bucket 100,000, notification job/payload/key drain 5,000, registration/deletion/permit/receipt Auth lifecycle 합계 100이다. family의 각 category와 합계가 모두 같은 상한을 지키고 staging은 각 2배 만료 fixture를 사용하며 reason별 overdue 0건을 release 기준으로 둔다. 50%에서 경보, 70% 또는 가정 변경 시 영업일 2일 안에 재검토, SLA 위반 예상 시 4시간 안에 signed catch-up과 신규 유입 제한을 적용한다.
3. `owner_id IS NULL`이고 capability 검증 owner activity 후 7일이 지난 draft·completed play는 `self_answers`, link, response와 token hash를 함께 cascade 삭제한다. owner read/save/complete/resume과 link create·rotate·disable만 activity를 갱신하며 visitor·OS share·Cron·실패 요청은 갱신하지 않는다.
4. 미제출 `visitor_responses.status = 'draft'`는 성공한 관계·시점/answer mutation의 `last_active_at`을 기준으로 24시간 뒤 assignment·answer·session hash와 함께 cascade 삭제한다. read·동일값 retry·실패 요청은 activity를 갱신하지 않는다. 제출 응답은 `submitted_at + 1년`, owner 삭제, 철회 중 먼저 온 시점까지 유지하되 만료된 `session_token_hash`는 `NULL`로 제거한다.
...
6. registration state가 있는 신규 Auth UID의 유일한 application adoption은 유효한 same-browser `answered` draft의 `claim_play`다. state가 존재하면 owner-session `create_or_resume_play`와 다른 모든 owner mutation을 거부한다. claim은 state가 transaction 안에 실제 남아 있고 `auth user created_at + 7일` grace 안이며 Auth deletion job이 아직 없을 때만 owner 귀속과 state 제거를 같은 transaction에서 완료한다. state가 없으면 claim을 거부하고, 일반 owner mutation은 기존 adopted-owner application anchor가 실제 있어야 한다. 따라서 Auth finish가 registration state와 UID를 지운 뒤 release된 cached claim/create 요청도 state 부재를 adoption으로 오인하지 않는다. grace가 지났거나 job이 enqueue·claim된 뒤에는 claim을 비노출 오류로 거부해 삭제 예정 Auth 사용자를 되살리지 않는다.
7. 일일 retention 고정 분기의 단일 `expire_retained_data`는 기존 table category와 함께 고정 `unclaimed_auth`, `auth_deletion_permit_prune`, `auth_deletion_receipt_prune` category를 공정하게 순회한다. 첫째는 정책 grace가 지난 registration state를 잠그고 `auth_deletion_jobs(reason = 'unclaimed_auth')`를 idempotent하게 만든 뒤 같은 transaction에서 state row를 삭제한다. 지정 UID는 active deletion job 한 곳에만 남고 provider 성공 finish에서 `NULL` 처리하며 grace 종료부터 앱 DB UID와 provider hard-delete를 모두 24시간 안에 완료한다. adoption과 enqueue가 같은 state를 잠그므로 grace 안 adoption이 먼저 끝나면 대상이 사라지고, grace 경과 또는 enqueue가 먼저 확정되면 adoption은 실패한다. 둘째는 released/expired permit을 `max(acquired_at + 5분, lease_until)` 뒤 24시간 안에 bounded·idempotent 삭제한다. 셋째는 receipt가 만료된 completed `owner_request`만 oldest-due-first로 골라 recovery actor hash/version·receipt hash/version과 status tombstone을 24시간 안에 함께 삭제하며, receipt가 유효하거나 non-terminal인 job은 건드리지 않는다.
...
14. `finish_auth_deletion_job`은 같은 proof로 active permit의 `released_at`을 기록한다. `not_called_call_window_expired`는 이미 증가한 attempt를 유지하되 provider failure와 별도 오류 code로 retry한다. 성공이면 job의 UID와 proof hash를 제거한다. `owner_request`는 pseudonymous recovery actor hash/version과 completed status tombstone·receipt hash/version을 receipt 만료까지 유지하고 그 뒤 24시간 안에 함께 bounded cleanup하며, `unclaimed_auth`만 즉시 최소 tombstone 삭제 대상이 될 수 있다. released permit은 job 완료·삭제와 무관하게 `max(acquired_at + 5분, lease_until)`까지 남아 rolling cap을 보존하고 그 뒤 24시간 안에 `auth_deletion_permit_prune`이 idempotent prune한다. Auth 삭제 직후 DB finish 전에 process가 중단돼도 permit slot은 lease 만료로 회수되고 stale job 재실행은 새 proof를 사용하며, rolling 5분 call count는 보수적으로 유지한 채 `user not found`를 성공으로 처리해 같은 완료 상태로 수렴한다.
15. 단일 `GET /api/internal/cron` dispatcher는 `CRON_SECRET`과 허용된 `X-Gyeop-Cron-Schedule`을 검증한다. reverse proxy는 이 path의 외부 접근을 거부하고 서버 cron만 app loopback port로 직접 호출한다. 5분 schedule의 Auth worker는 invocation당 claim 최대 10건·45초 예산 안에서 두 reason 합계 일일 peak 100건을 처리하되, 실제 Auth Admin call의 전역 rolling 5분 10회·lease형 동시 2회는 owner Route와 모든 Cron worker가 공유하는 permit으로 강제한다. 둘 다 due이면 첫 slot을 `owner_request`에 주고 같은 round에서 `unclaimed_auth`도 최소 한 건 처리하며, 한 reason이 비면 다른 reason이 몫을 빌린다. reason별 pending count·oldest due·overdue를 관측하고 둘 다 overdue 0건이어야 release한다. 요청 body·query에서 task나 UID를 받지 않고 Auth deletion RPC의 server secret role 외 실행 권한을 회수한다.
...
| `POST /api/plays` | cookie 없음 또는 anonymous-owner cookie | 없음이면 owner+요청 팩 draft 생성, valid면 같은 owner의 팩별 play를 생성·재개, malformed/stale/expired면 generic 404 |
| `GET /api/plays/[playId]` | anonymous-owner cookie 또는 fresh Auth session | capability owner 또는 Auth UID에 연결된 play의 셀프 선택·진행·완료 상태 복구 |
| `PUT /api/plays/[playId]/answers/[cardId]` | anonymous-owner cookie 또는 fresh Auth session | 소유한 draft 셀프 답변 idempotent 저장 |
| `POST /api/plays/[playId]/complete` | anonymous-owner cookie 또는 fresh Auth session | 정확히 10장 검사 후 `draft→completed`; incomplete·retry는 결정적 결과 |
| `DELETE /api/me/session` | optional anonymous-owner cookie | exact capability revoke 뒤 또는 missing/malformed 상태에서 idempotent 204와 cookie 삭제 |
| `GET /auth/google` | 완료 play+anonymous-owner cookie 또는 계정 로그인 요청 | 내부 return target 검증 뒤 private Supabase Auth Google OAuth PKCE 시작 |
| `GET /auth/callback` | PKCE code+10분 claim context+anonymous-owner cookie | code exchange와 fresh Auth actor 검증 뒤 owner의 모든 play를 한 UID에 claim하고 allowlisted 화면으로 이동 |
| `GET /api/me/plays` | fresh Auth session | 같은 UID에 연결된 팩별 play 목록 반환 |
| `POST /api/plays/[playId]/links` | fresh Auth session | 완료 play 소유권을 재검증하고 공개·1:1 링크 생성 |
| `GET /api/me/plays/[playId]/links` | fresh Auth session | sanitized 공개·1:1 링크 상태 목록; raw secret/hash 없음 |
| `PATCH /api/links/[linkId]` | fresh Auth session+body play ID | 현재 Auth owner play에 귀속된 링크 비활성화 |
| `POST /api/links/[linkId]/rotate` | fresh Auth session+body play ID | 기존 active 링크 비활성화와 같은 kind의 새 one-time URL 발급 |
| `POST /api/me/plays/[playId]/share-events` | fresh Auth session | exact `{event, linkId, entrySource}`를 받아 소유한 active 링크의 browser-reported share/copy 성공을 204/private no-store로 기록 |
| `POST /api/invites/[publicId]/metadata` | fragment secret, network+public ID 기준 60회/분 | owner 이름·답 없이 일반 팩 맥락과 링크 사용 가능 상태만 반환 |
| `POST /api/invites/[publicId]/responses` | fragment secret, rate limited anonymous | #22 public session은 valid existing session이면 같은 response·관계·시점을 복구하고 없으면 activity+24시간 session을 생성. #23이 public·1:1의 같은 assignment·saved progress 반환과 필수 3장 배정을 추가 |
...
### 11.1 익명 owner capability

- 가입 전 32-byte management secret을 `__Host-gyeop-owner` HttpOnly cookie로 보관한다. cookie는 호환 가능한 `v1.<anonymous owner UUID>.<base64url secret>` 형식이며 `Secure; HttpOnly; SameSite=Lax; Path=/; Max-Age=604800`을 사용하고 `Domain`을 두지 않는다.
- DB에는 `anonymous_owners`의 domain-separated SHA-256 hash와 `management_expires_at`만 권한 SSOT로 저장한다. owner 조회·저장·완료·로그아웃 RPC는 path play가 cookie owner에 속하는지와 secret hash, 만료·철회 상태를 같은 transaction에서 확인한다.
- 성공한 create·resume·read·save·complete는 owner의 DB 기준 7일 TTL을 갱신하고 같은 만료의 cookie를 다시 발급한다. 완료 후에도 같은 브라우저에서는 조회할 수 있지만 답은 수정할 수 없다. 미귀속 owner의 cookie가 없거나 만료·변조·철회되면 복구 수단은 없으며 다른 owner의 play를 제시해도 존재 여부를 드러내지 않는 404로 거부한다.
...
### 11.2 Google 로그인과 owner claim (active; 운영 확장은 inactive)

- P0의 사용자 로그인은 Supabase Google OAuth만 사용하며 카카오·네이버·비밀번호·이메일 매직 링크를 노출하지 않는다.
- `@supabase/ssr` PKCE를 사용하고 Next.js 서버가 session cookie를 갱신한다.
- owner-claim Google Route는 singleton `playId`·`returnTo`, 유효한 completed play와 anonymous capability를 provider 이동 전에 검증한다. 로컬·CI는 Google credential 없이 authorize redirect와 PKCE cookie를 확인하고, 실제 callback/claim 회귀는 production에서 닫힌 test-only Mailpit 경로로 검증한다. raw UID·Google email·이름·사진은 DB·analytics·app log에 추가하지 않는다. account-delete reauth branch는 inactive 후보다.
...
- owner claim은 인증을 시작한 같은 브라우저의 PKCE verifier, 10분 서명 claim-context cookie, 유효 anonymous-owner cookie, completed play가 모두 있을 때만 실행한다. 같은 owner의 모든 play를 복사 없이 한 Auth UID에 연결하며 같은 UID retry는 idempotent하고 다른 UID는 비노출 거부한다.
...
- owner cookie는 capability가 검증된 마지막 성공 owner read/write/link mutation부터 DB 서버 기준 7일, response session cookie는 마지막 성공 draft mutation부터 24시간이며 둘 다 `HttpOnly`, `Secure`, `SameSite=Lax`, `Path=/`다. submit은 response expiry를 `submitted_at + 24시간`으로 마지막 설정하고 이후 연장하지 않는다. cookie는 DB가 반환한 TTL과 expiry를 각각 `Max-Age`와 `Expires`에 사용하며 DB expiry만 최종 권한이다.
- 복원·변조한 owner cookie는 create/resume·save·complete에서 같은 비노출 오류로 거부한다. 만료된 anonymous play와 연결 데이터는 `docs/product/data-retention-and-deletion-policy.md`에 따라 24시간 안에 삭제한다. response session은 제출 때 회전하지 않고 결과·선택 응답 접근에만 사용하며 마지막 DB expiry 또는 철회 시점에 폐기한다. 만료 cookie를 복원·변조해도 서버는 answer·submit·continue·visitor comparison을 동일한 비노출 오류로 거부한다.
...
- 네트워크 식별자는 `RATE_LIMIT_SECRET`, UTC 날짜, 정규화한 IP를 HMAC 처리한다. bucket window 종료+24시간에 cleanup eligible이 되고 다음 24시간 안에 bucket과 함께 삭제한다.
...
- 연령·국가 확인은 제품 진입 조건으로 두지 않는다. 공식 질문팩은 전체 연령용 콘텐츠 기준을 지키고 생년월일·신분증·보호자 정보·IP geolocation은 수집하지 않는다.
- 현행 개인정보 법률 서면 검토, #7의 삭제·격리 restore drill, privacy 연락 채널이 완료되기 전에는 production beta를 열지 않는다.
...
방문자 경로의 초기 limit은 `invite metadata 60회/분/network+link`, `response 시작 10회/10분/network+link`, `answer 저장 120회/10분/response`, `submit 10회/10분/response`, `withdraw 5회/시간/network`다. 일반 Route Handler는 도메인 RPC 전에 `consume_rate_limit`을 호출하고, 초과 시 provider/domain 호출 없이 429와 정확한 `Retry-After`를 반환한다. account-delete magic-link context는 production candidate다. `start_response`는 같은 link의 유효·미만료 response session resume에서 quota를 소비하지 않고 기존 row를 반환한다. 새 row가 필요한 branch에서만 DB transaction 안에 `response_start` bucket 증가와 response insert를 원자 처리한다.
...
| 순서 | 초안 키 | 수직 결과 | 선행 조건 |
|---:|---|---|---|
| 1 | `POLICY-RETENTION` | 링크 만료·데이터 보관·완전 삭제 정책 확정 | 외부 제품·법률 결정 |
| 2 | `OPS-EMAIL-RUNTIME` | `$0` private MVP 운영 경계와 비활성 기능 확정 | 없음 |
| 3 | `DESIGN-RELATION-COPY` | 관계·알게 된 시점 code와 최종 한글 문구 | 없음 |
| 4 | `CONTENT-OLD-FRIEND-PACK` | 사람이 작성·승인한 오래된 친구팩 10장 | 사람 원문 입력 |
| 5 | `DESIGN-P0-SPEC` | 모바일 상태·token·레이어 UI 명세 | 3, 4 |
...
| 12 | `OWNER-DRAFT-AUTH` | anonymous-owner 다중 팩 capability·7일 same-browser 복구·공유 전 Google OAuth claim | 7, 8, 9, 10 |
...
| 24 | `STAGING-DEPLOYMENT-RUNBOOK` | local/CI·기존 Render Free 검증으로 재정의 필요 | 3, 7, 9, 10, 12~15, 19, 22 |
| 25 | `EMAIL-NOTIFICATIONS` | 비활성·연기: Resend outbox·worker를 배포하지 않음 | 별도 예산·재승인 |
| 26 | `FUNNEL-ANALYTICS` | 핵심 바이럴 event·금지 payload 회귀 검증 | 13, 16, 19, 21 |
| 27 | `DATA-RETENTION-CLEANUP` | DB 로직·local/CI 검증만 재검토; hosted scheduler는 연기 | 2, 8, 12, 14, 15, 17, 21, 26 |
| 28 | `SAFETY-OWNER-ACCOUNT-DELETION` | 비활성·연기: self-service/provider Auth deletion | 별도 예산·재승인 |
| 29 | `QA-MOBILE-ACCESSIBILITY` | 모바일 핵심 흐름·접근성 release gate | 11, 13, 16, 19~24, 28 |
| 30 | `QA-PERFORMANCE-RELEASE` | local/CI·Render Free best-effort 성능 gate | 8, 13, 19, 22, 24 |
| 31 | `QA-SECURITY-RELEASE` | 활성 Google OAuth·Render 경계와 inactive path fail-closed QA | 8, 9, 11, 12, 14~24, 27 |
| 32 | `PRODUCTION-RUNBOOK` | 비활성·연기: public production release | 별도 예산·재승인 |
...
실제 GitHub `status:*` label은 workflow truth이며 문서만으로 바꾸지 않는다. 특히 기존 #29 본문은 개인 서버·매직 링크 전제가 남아 있으므로 시작 전 `gyeop-issue-writer`와 task-harness 절차로 현재 SSOT에 맞춰야 한다. `$0` 단계의 #29·#30·#32·#33·#35·#36·#37 해석은 `docs/engineering/private-mvp-zero-cost-runbook.md`를 따른다.

## 18. 차단점과 후속 결정

현재 결정은 월 `$0`, 기존 무료 resource만 사용이다. 다음 production 후보는 삭제하지 않되 별도 예산과 external mutation 권한을 승인하기 전 활성화하지 않는다.

- 링크·owner·visitor·Auth·notification·analytics의 기간은 `docs/product/data-retention-and-deletion-policy.md`로 확정했다. 현행 한국 개인정보 법률 서면 검토와 provider backup 30일 증빙·격리 restore drill은 production release gate로 유지한다.
- 별도 staging/production, Auth custom SMTP, public OTP, Resend, 개인 서버 Cron·worker, self-service/provider Auth deletion과 public production

일반 보관·삭제 기간은 #7에서 확정했다. 다만 전체 연령용 질문팩 검수, 현행 개인정보 법률 서면 검토, provider backup 삭제·restore 증거, privacy 연락 채널은 production beta 차단점으로 유지한다.
...
비공개 재미 검증에는 아래 capability·core-flow와 공유 직전 Auth claim 기준을 적용한다. 이메일 알림·계정 삭제·미귀속 Auth cleanup 기준은 production beta 재승인 뒤 활성화하는 후보다.

현재 `$0` 완료 gate는 local/CI, 기존 Render Free·Supabase Free와 Google OAuth 경로에만 적용한다. 아래 staging/production/Cron/notification/Auth deletion 언급은 future production candidate이며 현재 기능이 준비됐다는 뜻이 아니다.
