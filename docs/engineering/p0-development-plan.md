...
Status: Reviewed
Review Status: PASS
Reviewer Agent: issue_backlog_review, development_plan_review
작성일: 2026-07-15
...
| Cron dispatcher·worker | 같은 internal RPC wrapper | 단일 Cron 인증과 허용 schedule 분기 뒤 expire·claim·resolve·prepare·limit·finish 등 schedule별 고정 allowlist만 호출 |
...
| preview | Vercel Preview; Cron 자동 실행 없음, signed manual smoke만 허용 | staging Supabase의 격리된 QA fixture |
| staging | 별도 Vercel project의 Production deployment | 별도 staging Supabase project |
...
- `CRON_SECRET` — Vercel이 모든 Cron invocation에 자동 전송하는 프로젝트별 서버 전용 bearer. staging·production 값은 분리
...
draft cookie는 `HttpOnly`, `Secure`, `SameSite=Lax`, `Path=/`, 7일 TTL이다. 이메일 인증 후 Route Handler가 `auth.getUser()`로 검증한 UID를 actor ID로 전달한다. `claim_play` RPC는 play row를 잠그고 `owner_id IS NULL`, `status = 'answered'`, token hash 일치를 다시 확인한 뒤 해당 actor ID에 귀속시키고 token hash를 비우며 cookie를 삭제한다. 이미 claim됐거나 token이 다르면 `409`다.
...
| `pack_plays` | `id`, `pack_version_id`, nullable `owner_id`, `draft_token_hash`, `draft_expires_at`, `status`, `answered_at`, `claimed_at`, `created_at`; 익명 draft 만료는 서버 시각으로만 판정 |
...
- `pack_plays.status`: 익명 완료는 `draft → answered`, claim은 `answered → active`; 이미 adopted owner가 직접 완료하면 `draft → active`; 이후 `active → archived`
...
1. 최초 생성은 `create_or_resume_play` RPC가 기존 draft를 확인하거나 play·draft token hash를 만들고 DB 서버 시각 기준 `draft_expires_at = clock_timestamp() + interval '7 days'`를 고정한다. cookie 만료는 이 시한을 넘지 않는다. owner session 경로는 lifecycle fence를 먼저 획득하고 registration/deletion guard를 통과한 뒤 기존 `owner_id = actor UID` application anchor가 실제 남아 있는 adopted owner에게만 허용한다. registration state가 없다는 사실만으로 adoption을 추정하지 않는다.
2. 익명 `create_or_resume_play`, `save_self_answer`, `complete_play`, `claim_play`는 모두 저장된 `draft_expires_at > clock_timestamp()`를 검사하고 만료·다른 token을 같은 비노출 오류로 거부한다.
3. 셀프 답변은 `save_self_answer` RPC의 upsert로 자동 저장한다.
4. `complete_play` RPC는 같은 pack version의 서로 다른 카드 10장을 확인한다. 익명 draft는 `answered`, 이미 인증된 owner play는 `active`로 바꾼다.
5. `claim_play` RPC는 lifecycle fence와 registration/play row lock, retained deletion guard, token·서버 만료·same-browser `answered` 검증 후에만 `owner_id`를 연결하고 `answered → active`로 바꾸며 draft token과 registration state를 폐기한다. registration state가 있는 신규 Auth UID가 application owner가 되는 유일한 경로이며 state가 이미 삭제된 cached request는 claim할 수 없다.
6. `owner_id`가 있고 `active`인 play만 링크 생성이 가능하다.
7. 링크에는 셀프 선택값을 포함하지 않는다.
...
5. 브라우저가 submit 직전 Web Crypto로 32-byte management token을 만들고 현재 브라우저 저장소에 pending 상태로 보관한 뒤 request body로 보낸다. RPC는 base64url 형식·길이를 검사하고 SHA-256 hash만 DB에 저장한다.
...
15. 단일 `GET /api/internal/cron` dispatcher는 `CRON_SECRET`과 허용된 `x-vercel-cron-schedule`을 검증한다. 5분 schedule의 Auth worker는 invocation당 claim 최대 10건·45초 예산 안에서 정책/OPS가 승인한 reason별 peak를 처리하되, 실제 Auth Admin call의 전역 rolling 5분 10회·lease형 동시 2회는 owner Route와 모든 Cron worker가 공유하는 permit으로 강제한다. `owner_request` 최소 reserved/priority와 `unclaimed_auth` 최소 진전을 보장하고 빈 quota는 서로 빌리되, reason별 pending count·oldest due·overdue를 관측해 둘 다 정책 물리 삭제 SLA 안에 수렴시킨다. 요청 body·query에서 task나 UID를 받지 않고 Auth deletion RPC의 server secret role 외 실행 권한을 회수한다.
16. 방문자 management token 철회는 owner 계정 삭제와 독립적으로 유지한다.

### 9.6 RPC 목록과 transaction 경계

아래 owner mutation의 session branch는 공통 `OwnerMutationActor` internal input(`uid`, retained reader별 `recovery_actor_candidates`)을 받는다. 각 RPC는 같은 transaction의 첫 domain 단계에서 lifecycle fence→retained owner-request tombstone→registration/adopted-owner anchor 순으로 검증하며, 익명 token branch와 명시된 read-only RPC에는 이 input을 받지 않는다. keyring reader 누락·후보 derivation 실패는 fail-closed다.
...
| `create_or_resume_play` | server wrapper | 익명 7일 draft 생성/재개; owner session은 공통 deletion guard 뒤 기존 owner anchor가 남은 adopted owner만 허용, state 부재만으로 adoption 추정 금지 |
| `get_owner_play` | server wrapper | draft token hash 또는 검증 owner actor 중 하나, 서버 TTL·owner 일치 검증 뒤 해당 play의 공개 카드 문구·셀프 선택·진행만 반환 |
| `save_self_answer` | server wrapper | 익명 token 또는 owner session 공통 deletion guard와 target owner anchor·서버 만료 검증 뒤 답변 upsert |
| `complete_play` | server wrapper | 익명 token 또는 owner session 공통 deletion guard와 target owner anchor·서버 만료·10장 검증 뒤 상태 전이 |
| `claim_play` | server wrapper | 공통 deletion guard와 row lock, same-browser answered token·서버 만료·현재 registration state 검증, 유일한 신규 Auth adoption, token·state 폐기 |
| `create_share_link` | server wrapper | 공통 deletion guard와 owner anchor·active play 상태 검증 뒤 public ID·secret hash 저장 |
| `rotate_share_link` | server wrapper | 공통 deletion guard와 owner anchor 검증 뒤 기존 링크 disable과 새 secret link 생성을 원자 처리 |
| `disable_share_link` | server wrapper | 공통 deletion guard와 owner anchor 검증 뒤 링크 상태 전이 |
| `list_owner_share_links` | server wrapper | owner actor·play 소유권 검증 뒤 `id/public_id/kind/status/expires_at/consumed_at`만 반환; secret/hash·응답 내용 금지 |
| `get_invite_metadata` | server wrapper | public ID·secret 검증과 일반 팩 맥락 반환 |
| `start_response` | server wrapper | 유효한 기존 response session이 같은 link면 response·assignments·저장한 방문자 선택을 idempotent 반환; 없을 때만 링크 검증, response·서버 24시간 만료 생성, 필수 3장 배정 |
...
| `POST /api/plays` | 없음, 기존 draft cookie 또는 registration state가 없고 current owner anchor가 실제 남은 adopted owner session | 새 draft 발급 또는 기존 draft 재개; 미귀속·삭제된 Auth session 거부 |
| `GET /api/plays/[id]` | draft cookie 또는 owner session | `get_owner_play`로 서버 TTL·소유권을 검증해 카드·셀프 선택·진행 복구 |
| `PUT /api/plays/[id]/answers/[cardId]` | draft cookie 또는 owner session | 셀프 답변 idempotent 저장 |
| `POST /api/plays/[id]/complete` | draft cookie 또는 owner session | 정확히 10장 검사 후 익명 draft는 `answered`, 이미 adopted owner play는 `active` |
...
| `POST /api/plays/[id]/links` | owner session | static config의 `default_share_kind`를 초기 추천으로 쓰되 owner가 확정한 공개·1:1 종류로 링크 생성 |
| `PATCH /api/links/[id]` | owner session | 링크 비활성화 |
| `POST /api/links/[id]/rotate` | owner session | 기존 링크 disable과 새 링크 발급 |
| `GET /api/me/plays/[id]/links` | owner session | sanitized 공개·1:1 링크 상태 목록; raw secret/hash 없음 |
| `POST /api/invites/[linkId]/metadata` | fragment secret, rate limited anonymous | 일반 팩 맥락과 링크 사용 가능 상태 |
| `POST /api/invites/[linkId]/responses` | fragment secret, rate limited anonymous | valid existing session이면 같은 response·assignment·saved choices 복구, 없으면 관계·시점 저장과 필수 3장 배정 |
...
| `GET /api/internal/cron` | `Authorization: Bearer CRON_SECRET` + 허용된 `x-vercel-cron-schedule` | `*/5 * * * *`에는 이메일·Auth deletion batch, `17 3 * * *`에는 보관 cleanup·미귀속 Auth enqueue·permit prune·expired completed owner receipt/recovery prune만 고정 분기 |
...
- draft, share secret, response session은 서버 `crypto.randomBytes(32)`의 base64url 값이다. management token은 브라우저 Web Crypto로 같은 32-byte 형식을 만든다.
...
- draft cookie는 7일, response session cookie는 24시간 TTL이며 둘 다 `HttpOnly`, `Secure`, `SameSite=Lax`, `Path=/`다. 각 cookie 만료는 DB 서버가 만든 `draft_expires_at`·`session_expires_at`보다 늦을 수 없다.
- 복원·변조한 만료 draft cookie는 create/resume·save·complete·claim에서 같은 비노출 오류로 거부한다. 만료된 anonymous draft 데이터는 retention cleanup이 cascade 삭제한다.
- draft cookie는 claim 직후 삭제한다. response session은 제출 때 회전하지 않고 결과·선택 응답 접근에만 사용하며 생성 후 서버 기준 24시간 TTL 또는 철회 시점에 폐기한다. 만료 cookie를 복원·변조해도 서버는 answer·submit·continue·visitor comparison을 동일한 비노출 오류로 거부한다.
...
- share·management fragment, draft·response·deletion-status cookie, raw receipt·proof, request body를 로그에 기록하지 않는다. path와 로그에는 secret이 아닌 public ID·내부 request ID만 남긴다.
...
- Cron은 일반 Preview에서 실행되지 않는다. SLA·중복·실패 격리 검증은 별도 staging Vercel project의 Production deployment에서 하고 Preview는 signed manual smoke만 수행한다.
- Vercel Instant Rollback은 active Cron schedule을 되돌리지 않으므로 rollback 직후 활성 두 schedule과 복구한 handler allowlist의 호환성을 확인한다. 불일치하면 Cron을 disable/update하거나 호환 `vercel.json`을 재배포한 뒤 5분·일일 분기를 각각 smoke하고, 비활성·오류 구간의 notification/Auth-deletion/retention 미처리 work를 안전하게 reconciliation한다.
...
`pack_opened.entry_source`는 `home | same_pack_cta`만 허용한다. `same_pack_start_clicked` 뒤 실제 전환은 새 owner flow의 `pack_opened.entry_source = same_pack_cta`로 확인한다. 공유 성공 event에는 link kind만 넣고 전체 URL·fragment secret·수신 채널을 넣지 않는다.

event에는 내부 UUID, link kind, pack version, relationship code처럼 허용된 값만 넣는다. 이메일, display name, IP, user agent 원문, token, A/B 선택은 넣지 않는다. 핵심 바이럴 funnel 검증은 owner→share→visitor→same-pack 실제 시작을 대조하고, 선택 2장·프로필 event는 각 기능 이슈가 같은 schema 검증을 소유한다.
...
초기 limit은 `owner draft 생성 5회/시간/network`, `magic_link_send 5회/시간/network+context`(context는 draft-claim의 draft 또는 account-delete의 verified owner 일일 HMAC), `invite metadata 60회/분/network+link`, `response 시작 10회/10분/network+link`, `answer 저장 120회/10분/response`, `submit 10회/10분/response`, `withdraw 5회/시간/network`다. 일반 Route Handler는 도메인 RPC 전에 `consume_rate_limit`을 호출한다. magic-link 두 branch는 같은 action·limit을 쓰되 context를 분리하고, 초과 시 provider 호출 없이 429와 정확한 `Retry-After`를 반환한다. 두 생성·재개 함수는 예외다. `create_or_resume_play`는 유효·미만료 cookie/owner draft resume, `start_response`는 같은 link의 유효·미만료 response session resume에서 quota를 소비하지 않고 기존 row를 반환한다. 새 row가 필요한 branch에서만 각 DB transaction 안에 `owner_draft_create` 또는 `response_start` bucket 증가와 play/response insert·assignment를 원자 처리하며, limit 초과면 domain row와 bucket 모두 commit하지 않는다. staging abuse test에서 정상 사용자 오탐이 1%를 넘으면 수치를 조정한다.
...
| 12 | `OWNER-DRAFT-AUTH` | 익명 owner draft·7일 복구·매직 링크 claim | 7, 8, 9, 10 |
...
| 17 | `VISITOR-CONTEXT-SESSION` | 관계·시점·24시간 visitor session | 4, 6, 14, 15 |
| 18 | `VISITOR-SAMPLING` | Signature 1장·최소 표본 2장 원자 배정 | 17 |
...
- staging·production Auth custom SMTP, public OTP residual abuse·미귀속 Auth 예상 생성량·beta 중단 기준, 발송 domain·한도와 Vercel production 요금제의 5분 Cron 지원 여부
...
- P1 Findings: 0
- P2 Findings: 0
- Review Notes: `development_plan_review`, `issue_backlog_review`, `final_parity`가 최신 문서와 32개 이슈 corpus를 독립 검토했다. 이슈 32/32, 의존성 116/116, 상태 blocked 6·ready 2·backlog 24, 이벤트 17/17, RPC 36/36, API 25/25 정합과 전체 검증 통과를 확인했다.
...
- [Vercel Cron Jobs 요청 계약](https://vercel.com/docs/cron-jobs)
