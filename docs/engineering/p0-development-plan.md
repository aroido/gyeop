...
|---|---|---|---|
| 비공개 재미 검증 | active | anonymous owner capability, 같은 브라우저 다중 팩·7일 inactivity 복구, 완료 뒤 이메일 매직 링크 claim, 인증 계정의 완료 play 조회 | 주인 응답·저장·공유, 방문자 비교·새 주인 전환, 누적 프로필 재공유의 재미 |
| production beta 재승인 | inactive candidate | public signup 운영, CAPTCHA·custom SMTP, 알림, 계정 삭제·미귀속 Auth cleanup | 전체 연령용 질문팩 검수, 현행 법률 서면 검토, 보관·backup·SMTP·운영 책임 재승인 뒤 별도 활성화 |
...
| 데이터·인증 | active: Supabase PostgreSQL, SQL migration, RLS, anonymous-owner capability cookie, Supabase Auth 이메일 PKCE claim | 익명 시작은 유지하고 공유 직전에만 계정 연결하며, 연결된 완료 play를 인증 계정에서 조회한다. |
...
비공개 재미 검증에서 active owner 경로는 `/`, `/play/new?pack=<slug>`, `/play/[playId]`, `/auth/sign-in`, `/auth/callback`, `/me`, `/me/plays/[playId]`, `/i/[publicId]#k=...`와 owner play·Auth magic-link·인증 play 목록·공유·프로필 API다. account-delete, notification/Cron 경로는 production beta candidate이며 현재 검증 gate에는 포함하지 않는다.
...
| `POST /api/auth/magic-link` | 완료 play+anonymous-owner cookie 또는 계정 로그인 요청 | 앱 5회/시간 rate limit 뒤 private Supabase Auth PKCE magic link 요청 |
...
### 11.2 이메일 로그인과 owner claim (active; 운영 확장은 inactive)

- P0는 Supabase 이메일 매직 링크만 사용한다.
- `@supabase/ssr` PKCE를 사용하고 Next.js 서버가 session cookie를 갱신한다.
- owner-claim magic-link Route는 `magic_link_send` fixed window `5회/시간/network+owner`를 외부 호출 전에 적용하고 유효한 completed play를 요구한다. private local MVP는 Mailpit으로 검증하며, public signup은 Supabase CAPTCHA·project email 한도·custom SMTP·운영 승인을 갖추기 전 열지 않는다. raw UID·email은 bucket·DB·analytics·log에 남기지 않는다. account-delete reauth branch는 inactive 후보다.
...
owner claim과 방문자 경로의 초기 limit은 `magic_link_send 5회/시간/network+owner`, `invite metadata 60회/분/network+link`, `response 시작 10회/10분/network+link`, `answer 저장 120회/10분/response`, `submit 10회/10분/response`, `withdraw 5회/시간/network`다. 일반 Route Handler는 도메인 RPC 전에 `consume_rate_limit`을 호출하고, 초과 시 provider/domain 호출 없이 429와 정확한 `Retry-After`를 반환한다. account-delete magic-link context는 production candidate다. `start_response`는 같은 link의 유효·미만료 response session resume에서 quota를 소비하지 않고 기존 row를 반환한다. 새 row가 필요한 branch에서만 DB transaction 안에 `response_start` bucket 증가와 response insert를 원자 처리한다.
