...
| production beta 재승인 | inactive candidate | 이메일 매직 링크 계정 연결, 알림, 계정 삭제·미귀속 Auth cleanup, cross-device 정책 | 확정된 만 19세·대한민국 정책의 #16 집행, 현행 법률 서면 검토, 보관·backup·SMTP·운영 책임 재승인 뒤 별도 활성화 |
...
- 최소 이용 연령은 `대한민국에서 이용하는 만 19세 이상`으로 확정했다. owner·visitor 공통 자기확인, 무보호자 동의, 최소수집, 신고 데이터 live 72시간·backup 30일·backup 밖 ledger 최소 45일 기준은 `docs/product/age-and-minor-policy.md`를 따른다.
- #16의 UI·API 집행, 현행 한국 개인정보 법률 서면 검토, #7의 삭제 ledger·격리 restore drill, privacy 연락 채널이 완료되기 전에는 production beta를 열지 않는다.
...
| 순서 | 초안 키 | 수직 결과 | 선행 조건 |
|---:|---|---|---|
| 1 | `POLICY-AGE` | 대한민국·만 19세·무보호자 동의 정책 확정 | 제품 결정 완료, 외부 법률 검토는 release gate |
...
최소 이용 연령·미성년자 정책은 #6, 일반 보관·삭제 기간은 #7에서 확정했다. 다만 #16 집행, 현행 한국 법률 서면 검토, provider backup 삭제·restore 증거, privacy 연락 채널은 production beta 차단점으로 유지한다.
