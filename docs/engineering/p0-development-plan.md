...
| 배포 | 비공개 재미 검증은 Render Free 단일 Docker web service, production beta 후보는 별도 승인 뒤 Linux host | 검증 환경은 Render `onrender.com` TLS URL 앞의 HAProxy와 loopback `next start` 한 인스턴스로 최소화한다. 기존 Linux `systemd`·nftables 운영 artifact는 production beta 후보용으로 보존하며, Render Free는 다중 instance·자동 public rollout을 열지 않는다. |
| 알림 이메일 | Resend REST (production beta 후보) | Google 로그인과 분리해 응답 도착 알림의 발송 경로·도메인·한도는 production beta 재승인 때 검증한다. |
...
| 2 | `OPS-EMAIL-RUNTIME` | Auth·이메일·Cron 실행 환경과 알림/Auth deletion 용량·비용 확정 | 외부 계정·비용 결정 |
...
| 12 | `OWNER-DRAFT-AUTH` | anonymous-owner 다중 팩 capability·7일 same-browser 복구·공유 전 이메일 claim | 7, 8, 9, 10 |
