...
각 단계는 앞 단계를 같은 owner 또는 response subject가 먼저 통과했을 때만 센다. `profile_reshare`의 마지막 단계는 profile-source 공유 성공 뒤 같은 공개 링크에 제출된 response가 있어야 한다. `visitor_same_pack`의 마지막 단계는 유효한 HttpOnly response capability, 제출 상태, session 만료, 같은 pack version을 DB에서 확인한 새 owner 생성만 센다.
