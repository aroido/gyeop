...
- `share_links.status`에 `consumed`를 추가하고 `consumed_response_id uuid unique`, `consumed_at timestamptz`를 추가한다.
  ...
  - consumed response가 해당 link에 속하는지는 submit transaction에서 잠금 아래 재검증한다.
- `start_response`와 invite metadata/list state를 교체해 consumed 1:1은 신규 start/metadata에서 기존 unavailable 경계로 수렴하고 owner link list에서는 `status='consumed'`, `consumedAt`만 보이게 한다. `consumed_response_id`는 owner/browser response에 노출하지 않는다.
...
- rate-limit key는 raw cookie/hash를 직접 bucket key로 쓰지 않고 domain-separated SHA-256(`action version + response UUID`) 32 bytes로 derive한다.
...
- Route는 raw management secret이 padding 없는 canonical base64url이며 decode 결과가 정확히 32 bytes인지 검사하고 domain-separated SHA-256 hash만 RPC에 전달한다. raw 값은 DB arg, log, analytics, response에 넣지 않는다.
- RPC는 hash가 정확히 32 bytes인지 검사하고 link row와 response row를 고정 순서로 잠근다.
...
- migration은 additive 후 constraint 교체라 배포 후 destructive down migration을 실행하지 않는다. 심각한 UI 회귀는 app을 직전 release로 rollback하고 신규 answer/submit route를 닫되 이미 submitted/consumed row를 draft로 되돌리거나 management hash를 삭제하지 않는다.
...
- rollback app이 `submitted|consumed`를 decode하지 못하는 기간에는 invite가 generic unavailable로 수렴하도록 fail closed하며, 실제 row를 삭제하거나 공개하지 않는다.
