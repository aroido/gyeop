...
`old-friend` 10장을 완료하고 재사용 공개 링크를 만든 owner가 현재 화면 memory에만 있는 전체 invite URL을 OS 공유 메뉴 또는 링크 복사로 실제 친구에게 전달하게 한다. 전달 성공과 취소·실패를 구분하고, reload로 raw secret을 잃은 상태에서는 공유 성공을 가장하지 않고 명시적인 재발급 경로만 제공한다.

이 PR은 첫 핵심 가설의 `셀프 10장 완료 → 공개 링크 생성 → 실제 외부 전달` 구간을 닫는다. 방문자 관계·응답 session은 #22, 3장 제출·비교·동일 팩 전환은 #24, 누적 프로필 재공유 진입은 #58이 소유한다.

## 범위

- `/me/plays/[playId]`의 공개 링크 생성·회전 직후 ready state에 native Web Share와 Clipboard API 동작을 추가한다.
...
- share resolve, share cancel, share failure, copy success, copy failure를 구분하고 실제 성공에서만 분석 event를 보낸다.
- reload·새 mount 뒤에는 list의 sanitized active link만 복구하고 share/copy controls를 표시하지 않는다. active 공개 링크 row에 `전체 링크가 사라져 새로 발급해야 공유할 수 있음`을 설명하고 기존 atomic rotate를 재발급 경로로 사용한다.
...
- `share_handoff_succeeded`, `share_link_copied` event에는 DB에서 확인한 exact `packVersion`, `linkKind=public`만 기록하고 URL·secret·channel·recipient를 받거나 저장하지 않는다.
...
- 1:1 링크의 외부 공유·복사 control과 1회 소비. #19의 lifecycle/ready URL 표시는 유지하지만 #21의 action/event는 `public` link에만 허용한다.
...
1. owner가 `/me/plays/[playId]`에서 `여러 친구에게 공개`를 선택하고 공개 링크를 생성하거나 active 공개 링크를 회전한다.
...
1. `링크 복사`는 Web Share 지원 여부와 관계없이 ready 공개 링크에 항상 보인다.
...
- active 공개 link row에는 `이 화면을 다시 열어 전체 링크가 사라졌어요. 공유하려면 새로 발급해 주세요.`와 기존 `새로 발급` action을 보여준다.
...
- action in-flight 동안 create/rotate/disable/share/copy를 모두 잠가 share sheet에 전달된 URL과 link 상태가 한 화면에서 경쟁하지 않게 한다.

### 1:1 링크

- `one_to_one` 생성·회전은 #19처럼 one-time ready URL과 상태 목록을 보여준다.
- #21의 native share/copy controls, 채널 안내, share event POST는 렌더·호출하지 않는다.
- 기존 active 1:1 lifecycle, owner override, reload sanitization을 회귀 테스트한다.
...
- ready public panel hierarchy는 `성공 heading → 한 줄 설명/추천 문구 → primary share → secondary copy → feedback → 수동 URL field` 순서다.
...
- URL은 readonly single-line field로 보여주고 horizontal page overflow 없이 field 내부 scroll/select가 가능해야 한다. raw URL을 React key, `data-*`, `title`, hidden input에 복제하지 않는다.
...
- share/copy가 끝나면 trigger에 focus를 유지한다. copy failure에서만 수동 field로 focus/select한다. create/rotate 성공은 ready panel heading에 focus해 새 action을 알린다.
...
- `share_handoff_succeeded|share_link_copied`는 새 RPC가 public link에서만 만들므로 실제 row의 `linkKind`는 `public`이다. policy는 공통 기존 event와 호환되도록 enum을 유지하고 RPC test가 public-only를 강제한다.
...
- `SECURITY DEFINER`, owner `gyeop_rpc_owner`, `search_path=''`, service_role execute-only 기존 계약을 따른다.
...
- authorization success 뒤 `(pack_play_id, link_id)` row를 `FOR UPDATE`로 잠근다. effective expiry를 `expired`로 수렴시킨 뒤 exact `kind='public' and status='active'`일 때만 event를 insert한다.
- missing/cross-play/disabled/expired/one-to-one은 event 0건이고 public route에서 generic owner 404로 수렴한다.
...
- missing/malformed/expired/tampered/cross-play/inactive/non-public link는 기존 owner generic 404 body/header와 cookie 보존·삭제 규칙을 따른다.
...
- analytics POST는 browser action success를 바꾸지 않는 best-effort side effect다. action feedback을 먼저 확정하고 rejection은 swallow하되 console/error telemetry에 request 값을 기록하지 않는다.

## 구현 계획

1. `20260718000500_share_handoff_events.sql`에 policy 교체와 `record_owner_share_action`을 추가하고 pgTAP으로 public active success, 1:1/cross-play/inactive failure, exact payload, grant/RLS를 고정한다.
...
5. `ShareLinkManager`의 raw ready state를 link-bound object로 바꾸고 public-only share/copy UI, reload guide, focus/live feedback, action concurrency를 구현한다.
...
8. 320/390/430px와 Web Share 지원/미지원, clipboard 실패 screenshot/browser QA를 수행하고 report 권고와 실제 화면 hierarchy를 대조한다.
9. `docs/engineering/p0-development-plan.md` route/RPC/event/UI 계약을 실제 구현과 맞춘다.
10. `./scripts/run-ai-verify --mode full`을 통과한다.

## 완료 기준

- [ ] public create/rotate success 직후 current mounted screen에만 share/copy controls가 보이고 exact invite URL을 사용한다.
...
- [ ] DB RPC는 completed same-play owner capability와 active public link에서만 event를 기록하고 1:1/cross-play/disabled/expired/tampered 요청은 mutation 0이다.
- [ ] event row properties는 DB-derived exact packVersion/linkKind만 포함하며 link kind는 public이다.
- [ ] reload 뒤 active public link가 있어도 raw URL/share/copy control이 없고 재발급 안내와 atomic rotate만 제공한다.
- [ ] ready link disable은 raw URL을 clear하고 rotate success는 old URL을 폐기해 새 URL 하나만 유지한다.
- [ ] 1:1 lifecycle/UI에는 #21 analytics/action이 생기지 않는다.
- [ ] share/copy/action 경쟁 click이 중복 browser API 호출이나 stale link event를 만들지 않는다.
- [ ] 320/390/430px에서 가로 overflow가 없고 44px target, keyboard tab, visible focus, heading/feedback focus, reduced motion이 동작한다.
...
- active public same-play 두 event success와 exact properties
- one-to-one, disabled, expired, cross-play, malformed event, tampered capability event 0
...
- missing/malformed/tampered/cross-play/inactive/non-public generic owner boundary와 event 0
...
- rapid double click과 share in-flight 중 create/rotate/disable/copy 잠금
- reload 뒤 full URL/code/share/copy 없음, active public 재발급 안내, rotate 뒤 새 controls 복구
- current ready disable clear, another create/rotate ready replacement
- one-to-one ready/list 회귀와 public event/action 부재
- 320/390/430 mobile overflow, 44px, keyboard, focus, live region, reduced motion

### live Supabase E2E

- completed owner → public create → stubbed native share resolve → 실제 event row exact 1건
- copy resolve → 실제 event row exact 1건
- cancel/failure와 one-to-one에 event 0
...
- 둘 다 exact `packVersion`, `linkKind=public`만 DB에 기록한다.
...
- 브라우저 성공 뒤 analytics POST가 실패해도 실제 사용자 성공 feedback은 유지한다. event 수집 실패를 공유 실패로 거짓 표시하거나 자동 재시도로 중복 집계하지 않는다.
...
- browser는 link kind/packVersion을 analytics truth로 보내지 않는다. server가 owner capability, play, active public link를 확인해 DB row에서 계산한다.
- event endpoint는 valid owner capability라도 다른 play/link, inactive/expired link, 1:1 link에 사용할 수 없다.
...
- #21은 public link만 실제 외부 전달 가설로 측정한다. 1:1 외부 전달 control은 P0 재미 검증 결과 뒤 별도 재승인한다.
