...
비공개 MVP 공식 팩 4종의 제목을 `상대가 보는 나`라는 GYEOP의 핵심 보상이 한눈에 읽히는 관계형 `시선` 시리즈로 바꾸고, 홈 선택부터 공유·비교·동일 팩 시작·프로필까지 같은 제목을 일관되게 사용한다.
...
| ------------------ | --------------------- | ----------------- | -------------- | ------ | --------- | ----------------------------------- |
| `old-friend`       | `오래 본 너의 시선`   | 오래된 친구       | 따뜻한 회상    | 낮음   | 공개      | 오래 지켜본 친구에게 비친 현재의 나 |
| `first-impression` | `처음 만난 너의 시선` | 새로 알게 된 사이 | 가벼운 첫 만남 | 낮음   | 공개      | 첫 만남에서 상대에게 보인 나        |
| `coworker`         | `같이 일한 너의 시선` | 직장 동료         | 담백한 관찰    | 낮음   | 공개      | 함께 일한 동료가 관찰한 업무 속 나  |
| `honest-self`      | `가까운 너의 시선`    | 가까운 사이       | 차분한 솔직함  | 중간   | 1:1       | 가까운 관계에서만 드러나는 나       |
...
- [ ] 레이아웃과 시각 토큰은 변경하지 않는다. 새 제목 길이는 각각 공백 포함 9~11자로 기존 카드의 2줄 제목 영역 안에 들어간다.
...
- [ ] `honest-self`의 제목 `가까운 너의 시선`은 연인만을 뜻하지 않도록 홈 관계 label `가까운 사이`와 함께 표시한다.
