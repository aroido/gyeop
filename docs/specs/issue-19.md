...
`old-friend` 10장을 완료한 owner가 현재 play에 귀속된 공개 또는 1:1 초대 링크를 만들고 외부로 실제 공유할 수 있게 한다. URL은 `/i/{public_id}#k={secret}`으로 고정해 bearer secret이 HTTP request path, reverse-proxy access log, DB, analytics에 남지 않게 하고, 같은 공개 링크는 독립 방문자 여러 명이 안전하게 열 수 있게 한다.

이 PR은 `owner 10장 완료 → 링크 생성·복사/공유 → 방문자 일반 초대 화면 진입`까지를 실제 제품에서 닫는다. 방문자의 관계 선택·3장 응답·제출은 #21 이후가 소유한다.
...
- 공개 ID와 secret을 CSPRNG로 생성하고 raw secret은 app process memory와 생성/회전 HTTP 응답에만 한 번 존재하게 한다. DB에는 domain-separated SHA-256 hash만 저장한다.
...
- `/me/plays/[playId]`에 링크 종류 선택, 생성 직후 한 번만 보이는 전체 URL, 복사/Web Share, 상태 목록, 비활성화, 회전을 구현한다.
...
- 방문자 관계·알게 된 시점 입력, 3장 배정/저장/제출, 비교 결과와 방문자 session. #21~#24가 소유한다.
- 1:1 링크 소비와 `consumed_response_id` FK 연결. 이번 migration은 nullable future column과 상태 계약만 준비하고 어떤 RPC도 `consumed`로 전이하지 않는다.
...
- visitor가 fragment를 응답 session cookie로 교환하는 동작. #21의 `start_response`가 소유한다.
...
이번 구현에서 문서 의미를 바꾸는 새 제품 결정은 없다. 구현 후 `p0-development-plan.md`의 URL/API/RPC 표가 실제 route와 일치하지 않는 항목만 exact 갱신한다. `core-feature-priority.md`의 카카오톡·Instagram 등 채널 지원 문구는 P0 방향으로 유지하되, 이번 PR의 실제 handoff는 표준 Web Share와 link copy임을 개발 계획에 명시한다.

Lazyweb의 `mobile invite link sharing` 실제 화면 검색에서 공통적으로 확인된 패턴을 적용한다.

- 전체 URL보다 `공유하기`를 primary action, `링크 복사`를 항상 가능한 fallback으로 둔다.
...
4. 생성 요청이 성공하면 전체 `/i/{publicId}#k={secret}` URL을 해당 mounted 화면 memory에만 보관한다. `공유하기`는 Web Share API를 사용하고, 지원하지 않거나 취소된 경우에도 `링크 복사`를 사용할 수 있다.
5. clipboard/share 성공 여부는 UI 상태로만 안내하고 URL이나 secret을 analytics/log에 보내지 않는다.
6. 새로고침하면 전체 URL은 사라지고 link 종류·상태·만료·소비 시각만 다시 보인다. active link에는 `새로 발급`, 모든 상태에는 필요 시 `새 링크 만들기`를 제공한다.
7. `새로 발급`은 old active link 비활성화와 같은 kind의 new link 생성을 한 transaction으로 처리한다. 성공한 새 URL만 한 번 표시한다.
8. `비활성화`는 확인 dialog 뒤 실행한다. 성공 뒤 기존 URL을 memory와 DOM에서 즉시 제거하고 상태를 `비활성`으로 표시한다.
9. draft play, 다른 play path/cookie, 만료·변조 capability는 동일한 종료 화면으로 수렴하며 다른 play/link 존재를 드러내지 않는다.
...
- `public_id`: CSPRNG 16 bytes를 padding 없는 base64url로 인코딩한 exact 22자 `[A-Za-z0-9_-]{22}`.
...
- `id uuid primary key`
- `public_id text not null unique` + exact 22-char base64url CHECK
...
- `status text not null default 'active' check (status in ('active','consumed','disabled','expired'))`
- `expires_at timestamptz null`
- `consumed_response_id uuid null`
- `consumed_at timestamptz null`
- `created_at timestamptz not null default clock_timestamp()`
- `updated_at timestamptz not null default clock_timestamp()`
- `consumed` 상태만 consumed 두 field가 non-null이고 다른 상태는 둘 다 null인 CHECK를 둔다. #21이 `visitor_responses`를 만든 뒤 exact FK를 추가한다.
...
- allowed transition은 #19에서 `active→disabled`, `active→expired`뿐이다. `consumed` 전이는 #21만 추가한다.
- disable retry는 이미 disabled면 같은 row를 반환하는 idempotent success다. expired/consumed link는 변경하지 않고 현재 상태를 반환한다.
...
- helper가 authorized가 아니면 그 outcome을 그대로 내부 반환하고 link/play를 추가 변경하지 않는다.
...
- internal success envelope는 renewed `managementExpiresAt`, `managementTtlSeconds`를 포함할 수 있지만 HTTP JSON에서는 link allowlist만 노출하고 같은 raw owner cookie value의 expiry만 갱신한다.
...
- active는 disabled로 전이하고 already disabled는 idempotent success다. expired/consumed은 상태를 바꾸지 않고 authorized current link를 반환한다.
...
- HTTP에 노출 가능한 link shape는 exact `id/publicId/kind/status/expiresAt/consumedAt`뿐이다.
...
- active+future/null expiry만 `active`다. past expiry는 row를 expired로 수렴하고 `unavailable`을 반환한다. disabled/expired/consumed도 동일한 `unavailable` external mapping이다.
...
- `lib/share-links/share-link-session-core.mjs`
  - `node:crypto`를 사용하는 server-only credential 생성/hash/fragment parser URL helper.
...
- missing/malformed/expired/tampered/cross-play/path-link mismatch/not-completed/link-not-found는 status 404와 exact owner error body로 수렴한다. link row mutation은 0회다.
- cross-play valid cookie는 `Set-Cookie`로 지우거나 갱신하지 않는다. link owner failure 전체는 byte-identical body와 no-store를 유지하고 secret/link 상태를 노출하지 않는다.
...
- invite route는 canonical public id로 `deriveInviteRateLimitKey(networkKey,publicId)`를 만든다. domain-separated SHA-256 결과만 `consume_rate_limit`에 전달한다.
...
- 상단에는 `내 답변` back link, `공유 링크` heading, 완료한 pack title을 둔다.
- 링크 종류는 두 개의 44px 이상 radio-card로 제공한다.
  - `여러 친구에게 공개` — 여러 명이 참여할 수 있음
  - `한 친구에게 1:1` — 한 명 완료 뒤 닫힘(소비 동작은 #21)
...
- invite URL이 memory에 있을 때 `공유하기` primary, `링크 복사` secondary, one-time 안내 `이 링크는 지금만 다시 복사할 수 있어요`를 보여준다.
- Web Share unsupported/AbortError는 실패 toast로 과장하지 않고 copy fallback을 유지한다. clipboard denied/network error는 `복사하지 못했어요 · 다시 시도`를 `aria-live=polite`로 표시한다.
- 상태 목록은 kind, active/disabled/expired/consumed, nullable expiry/consumed time만 보여준다. secret placeholder나 일부 masking도 표시하지 않는다.
...
- #21 전까지 CTA는 disabled fake button을 두지 않고 `친구 답변은 다음 단계에서 이어져요` 안내만 둔다. #21이 같은 screen에 관계/3장 시작을 연결한다.
...
6. `/me/plays/[playId]` link management UI, #18 completed CTA, `/i/[publicId]` fragment metadata UI를 구현한다.
7. focused unit/pgTAP/integration/Playwright를 추가하고 실제 local Supabase+Chromium에서 create→copy URL→새 context invite open→rotate/disable/reload를 검증한다.
8. 실제 390×844 screenshot을 Lazyweb/visual QA로 확인하고 overflow/focus/copy를 보정한다.
...
- disable idempotency, rotate atomicity, concurrent rotate one winner
...
- completed owner 화면→link manager→default public 생성→copy/share fallback
...
- copy/Web Share 결과는 이번 PR에서 server event로 보내지 않는다. public action으로 확인할 수 없는 성공을 과대 집계하지 않는다.
...
- raw secret은 process memory에서 response 생성 직후 참조를 버리고 재조회/복구 기능을 제공하지 않는다.
...
- 1:1 `consumed_response_id`의 FK와 실제 consume 전이는 #21의 visitor response schema와 같은 migration에서 완성한다.
- Web Share의 플랫폼별 channel 우선순위는 미결정이며 native share sheet에 맡긴다. copy는 항상 fallback이다.
