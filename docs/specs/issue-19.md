...
- #19의 status CHECK는 `active|disabled|expired`만 허용한다. #24가 `visitor_responses`와 함께 `consumed_response_id`, `consumed_at`, exact FK를 추가하고 `consumed`를 확장한다. 그 확장 constraint는 `consumed ⇒ kind=one_to_one and consumed fields non-null`, 다른 상태에서는 consumed fields null을 강제한다.
...
- 기존 `analytics_events`에는 `gyeop_internal_rpc` INSERT만 추가 grant한다. `analytics_share_invite_internal_insert` policy는 event name이 `share_link_created|invite_opened`이고 properties key가 exact `packVersion|linkKind`, linkKind가 exact enum일 때만 INSERT를 허용한다. role에는 analytics SELECT/UPDATE/DELETE를 주지 않는다.
...
| limiter/internal error | 500 | 0 | 변화 없음 | 없음, 정상 cookie 보존 |
