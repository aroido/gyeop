...
- [ ] `/i/[publicId]` 첫 HTML에 초대 맥락을 서버 렌더링하고 `generateMetadata` 및 1200×630 `ImageResponse`를 닉네임·안전한 팩 맥락·presentation으로 생성한다. OG의 한국어와 허용 닉네임 glyph는 로컬 OFL Noto Sans KR weight 900 static subset TTF로 렌더한다.
...
- [ ] 1:1·민감·fallback OG는 팩 제목/recipe 없이 같은 추천 문구와 중립 GYEOP 표현만 사용한다. 공식 Google Fonts의 Noto Sans KR variable source를 weight 900으로 고정한 OFL subset TTF와 CSS 도형만 사용하고 runtime network fetch는 하지 않는다.
...
- [ ] `app/i/[publicId]/assets/`에 `NotoSansKR-InviteSubset.ttf`, `OFL.txt`, `README.md`를 두고 README에 provenance/hash/static instancing/subset range와 재생성 명령을 기록한다. `opengraph-image.tsx`는 번들된 TTF ArrayBuffer를 한 번 읽어 `ImageResponse`의 `fonts` option에 전달하고 runtime network 요청 없이 rich PNG를 생성한다. 가변 WOFF는 Next `ImageResponse` parser에서 실제 fallback 회귀를 일으켜 사용하지 않는다.
...
- [ ] 별도 runtime/dev dependency나 자체 cmap parser는 추가하지 않는다. provenance의 deterministic static instancing/subset command에 U+AC00-D7A3/U+0041-005A/U+0061-007A/U+0030-0039/exact copy 범위를 명시하고, TTF SHA-256 고정·실제 `ImageResponse` 실행 테스트와 `가힣AZaz09` PNG boundary visual check로 이번 private-MVP coverage를 검증한다. 허용 범위를 넓힐 때는 cmap 자동 검사를 별도 검토한다.
