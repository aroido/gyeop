...
- [x] raw analytics event는 30일, 비식별 집계는 1년 유지한다. rate-limit bucket은 window 종료+24시간에 cleanup eligible이 되고 다음 24시간 안에 hard-delete한다. source 삭제 시 raw event subject ID/property는 즉시 비식별화한다.
...
- [x] grace 종료 cleanup은 registration state를 잠가 `unclaimed_auth` job을 만들고 같은 transaction에서 state row를 삭제한다. 그 뒤 지정 UID는 active deletion job 한 곳에만 남고 provider 성공 finish에서 24시간 상한 안에 `NULL` 처리한다.
...
| retention family와 포함 category | production 승인 일일 peak | 2배 staging fixture | 근거 |
|---|---:|---:|---|
...
| Auth lifecycle: registration, 두 deletion reason, permit, receipt | 100 | 200 | 승인안의 미귀속 Auth·owner 삭제 합산 상한 |
| notification lifecycle: job, payload version, key drain | 5,000 | 10,000 | 승인안의 terminal notification 상한을 family 전체에 적용 |

peak는 관측된 성장 예측이 아니라 beta admission ceiling이다. family의 각 포함 category와 합계가 모두 같은 상한을 넘지 않아야 하며 한 category의 빈 몫만 같은 family에서 빌릴 수 있다. 운영자가 이 상한을 강제하고, 실제 유입이 70%에 닿거나 가정이 바뀌면 Product Owner와 Operations Owner가 영업일 2일 안에 재승인한다. worker batch/RPS는 #8·#29가 이 2배 fixture를 24시간 SLA 안에 drain하도록 별도 증명한다.
...
- [x] 미귀속 registration state→deletion job UID→provider hard-delete/UID NULL의 transaction 순서와 각 물리 삭제 상한이 명시된다.
...
Reviewer Agent: issue7_spec_review
Review Status: PASS
P0/P1 Findings: 0
