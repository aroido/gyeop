...
- 팩 주인은 결과를 저장할 때 이메일 매직 링크로 계정을 만든다.
- P0 로그인과 draft 귀속은 답변을 시작한 같은 브라우저에서 매직 링크를 열 때만 지원한다. 다른 기기에서는 로그인을 진행하지 않고 원래 브라우저로 돌아가도록 안내한다.
...
P0 알림은 계정 이메일로만 보낸다. 웹 푸시와 친구에게 재촉하는 자동 알림은 제외한다.

### 5.9 계정 삭제와 보관

- 인증된 팩 주인은 자신의 계정 삭제를 요청할 수 있어야 한다.
- 계정 삭제는 해당 주인의 팩 플레이, 셀프 응답, 공유 링크, 연결된 방문자 응답과 application 식별자를 확정된 보관 정책에 따라 제거한다.
...
- 인증된 주인은 계정 삭제를 요청할 수 있고, 확정된 보관 정책에 따라 live application 데이터와 backup 완전 삭제 시한이 추적된다.
