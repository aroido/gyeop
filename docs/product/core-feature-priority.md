...
계정 통합 `/me` 상단의 `질문팩 공유하기`는 완료 play가 있으면 submitted 공개 링크 시선이 0건이어도 표시하는 초기 공유 진입이다. query나 event를 추가하지 않고 해당 play의 공유 관리 화면으로 이동하며, 실제 링크 복사·공유는 그 화면에서 수행한다. 이는 아래의 per-play `시선 더 모으기` 재공유와 별개다.
