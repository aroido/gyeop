...
첫 P0 테스트에서는 운영자가 만든 `오래된 친구팩` 1개만 제공한다. 아래 팩은 핵심 루프 지표를 확인한 뒤 순차적으로 검토한다.
...
비공개 재미 검증의 `/me`는 submitted 공개 링크 응답만 전체 시선과 카드 표본에 포함한다. 1:1 응답은 방문자 본인의 즉시 비교에만 남기고 `/me` 누적에서는 제외한다. 관계별 레이어와 private 전체 시선의 1:1 포함 여부는 production beta 재승인 전에는 구현하지 않는다.
...
P0 첫 공식 팩과 타깃 관계는 **오래된 친구**로 확정한다. 관계 맥락이 명확하고, 썸·연애보다 공유 장벽이 낮으며, 직장동료보다 감정적인 결과가 잘 나오기 때문이다. 다른 공식 팩은 이 팩의 핵심 루프 지표를 확인한 뒤 순차적으로 검토한다.
