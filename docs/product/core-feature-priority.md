...
- same-browser owner는 로그아웃으로 현재 play 관리 hash와 cookie를 즉시 폐기한다. 만료 row의 물리 삭제는 확정된 보관 정책과 cleanup 이슈가 담당한다.
- Auth 계정 삭제가 필요한 production beta는 재인증, 해당 주인의 팩 플레이·셀프 응답·공유 링크·연결 방문자 응답과 application 식별자 제거를 별도 재승인한 뒤에만 연다.
...
- 링크 만료, 데이터별 보관 기간, backup 완전 삭제 시한이 확정되기 전에는 production beta를 열지 않는다.
...
- production beta는 현행 한국 개인정보 법률 서면 검토, #16 UI/API 우회 차단, #7 live 72시간·backup 30일·ledger 45일/restore drill, 공개 privacy 문의 채널 없이는 열지 않는다.
