...
`내 겹 공유하기`는 공개 프로필을 만들지 않는다. 주인이 확인한 관계 하나와 질문 하나의 원본 문구·선택지·셀프 선택·A/B 수치만 1080×1920 PNG로 브라우저에서 만들고, 기존 public 질문팩 초대 URL과 함께 OS 공유를 연다. 관계와 질문 표본이 각각 3명 이상인 값만 사용하며 `romantic`, 1:1 응답, 닉네임, 내부 ID, 관리 URL과 secret은 카드 데이터에서 제외한다. 파일 공유를 지원하지 않거나 실패하면 이미지 저장과 링크 복사를 각각 제공한다.
