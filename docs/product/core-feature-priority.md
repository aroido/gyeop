...
- 셀프 10장을 완료하고 공유하려는 시점에만 Supabase Auth Google OAuth를 요청한다. 같은 브라우저에서 Google 로그인을 마치면 익명 owner의 모든 play를 Auth 계정에 원자적으로 연결한 뒤 공유 화면을 연다. 카카오·네이버·비밀번호·이메일 매직 링크 로그인은 사용자 경로에 제공하지 않는다.
- 계정에 연결된 play 목록·답변·공유 관리·누적 프로필은 같은 Google 계정으로 다른 브라우저에서도 다시 열 수 있다. 연결된 draft는 남은 답을 이어 저장·완료할 수 있다. 서버는 fresh Auth UID와 play 소유권을 매 요청에서 함께 검증하며 anonymous capability secret을 브라우저에 재발급하지 않는다. Google 계정 이메일·이름·사진은 친구나 방문자에게 노출하지 않는다.
- 첫 Google 로그인 뒤에는 초대장에 쓸 공개 닉네임을 필수로 받는다. 닉네임은 NFKC 정규화 뒤 한글 완성형·영문·숫자와 단어 사이 한 칸만 허용하는 2~12자이며, 완료 전에는 `/me`와 링크 생성·회전에 진입할 수 없다.
...
현재 비공개 재미 검증에서는 사람이 직접 작성·검수한 공식 팩 24개를 제공한다. 모두 10장 A/B 카드와 Signature 1장을 가지며, `content/packs/*-vN.json`이 버전별 제목·문구·공유 권장의 SSOT이고 slug별 가장 높은 버전을 새 play에 사용한다.

- 관계의 시선: `우리는 아직도 통하는 편`, `첫 장면, 네 버전`, `퇴근 전의 우리`, `말 안 해도 알까?`, `우리 둘이 섞인다면`
- 온라인·대화: `읽고도 생각 중`, `단톡방의 나`, `알고리즘이 들킨 날`, `이모지 자막 켜기`, `내적 댓글창`
- 웃긴 일상: `주말 사용 설명서`, `여행 가방의 철학`, `최근 사진 20장`, `간식 취향 보고서`, `웃음 버튼의 위치`, `갑자기, 지금, 같이?`
- MBTI풍 상황 선택: `사람들 사이 배터리`, `결정은 내일의 나에게`, `칭찬 영수증`, `혼자만의 루틴`, `방 온도와 내 기분`
- 생활 모드: `마감 전의 나`, `작은 사치 연구소`, `퇴근 후 본캐`

이 활성화는 팩별 재미와 공유 전환을 비교하기 위한 private MVP 범위다. MBTI 코드·고정 성격 결과는 만들지 않고, 온라인 유행의 형식만 독자적인 상황 A/B 질문으로 재해석한다. 다음 공식 팩 추가와 public beta 노출은 이 24팩의 핵심 루프 지표를 확인한 뒤 별도 검토한다.
...
- 팩 생성·재개 요청과 동시에 한 화면의 자체 카드팩을 보여 주고, 모바일에서 약 220px 이내로 위로 밀면 봉인이 열리고 나온 카드가 첫 질문으로 이어진다. 이는 별도 확인·선택 화면이 아니라 첫 질문 reveal이며, 85% 이후에는 끝까지 정착한다. 키보드·보조기기용 `팩 열기` 버튼을 함께 제공하고 `prefers-reduced-motion: reduce`에서는 입력이나 지연 없이 즉시 질문으로 이동한다. 외부 이미지·특정 IP·소리는 사용하지 않는다.
- 한 화면에는 카드 한 장만 보여준다.
...
- 완료 화면의 `내 질문팩 저장하고 공유하기`에서 Google OAuth로 owner를 계정에 먼저 연결한다.
- 로그인·claim이 끝나지 않은 owner는 링크를 생성·조회·회전·비활성화할 수 없다.
- 새 링크는 생성 당시 닉네임을 snapshot으로 저장한다. 이후 계정 닉네임을 바꿔도 기존 링크 문구는 바뀌지 않으며, rotate는 현재 닉네임을 새 링크에 다시 snapshot한다.
...
- 링크를 열면 secret fragment를 읽기 전 server HTML과 SNS 미리보기에 `{닉네임}님이 먼저 답한 질문팩이에요` 맥락을 보여준다. 1:1 또는 medium/high 팩은 닉네임만 쓰고 팩 제목·정체성은 숨기며, snapshot이 없는 기존 링크는 기존 generic 문구를 유지한다.
- 이 닉네임은 검색·공개 프로필이 아니라 링크를 가진 사람이 보는 crawler-visible 공개 정보다. private MVP에서만 명시적으로 승인한 예외이며, 신고·금칙어·운영 moderation이 준비되기 전에는 public beta를 열지 않는다.
...
인증된 owner의 `/me` 첫 정보 구조는 질문팩 목록이 아니라 owner가 직접 입력한 닉네임을 제목으로 한 계정 통합 프로필이다. `GYEOP_CONCEPT_PROFILE_ENABLED=true`에서는 완료한 concept v1 팩의 여러 장면을 8개 영역·32개 양방향 결로 연결해 대표 결 3개를 먼저 보여 준다. 각 대표 카드는 영역·결 이름, A/B 양쪽 끝점, 내 위치, 공개 기준을 통과한 지인 익명 집계, 고유 문항 수와 근거 단계로 구성하며 자연어 성격 해석문을 1차 결과로 사용하지 않는다. 결은 사람 전체를 규정하는 유형·진단·점수가 아니다. feature가 꺼져 있거나 source pair가 없으면 기존 관계 레이어 프로필을 그대로 유지한다.
...
- 대표 결 정확히 3개: `difference → contextual → repeated → emerging`의 기존 one-per-kind 순서와 rank를 지키며 가능한 한 서로 다른 영역을 우선
- 양방향 위치: catalog의 A를 왼쪽, B를 오른쪽에 두고 public `position`만 표시
- 근거 상세: `왜 이렇게 보일까?`에서 팩 제목과 검수된 맥락만 표시
- 내가 보는 나와 주변 시선: 서로 합산하지 않은 별도 근거
- 부족 상태: 임계값을 넘기 전에는 `시선을 모으는 중 · n/3` 외 위치·방향·범위를 payload와 DOM에서 완전히 제외
- 8개 영역 rail: self 질문을 같은 `cardKey` 기준으로 영역 안에서 한 번만 세고 전면에는 고유 문항 수와 단계만 표시하며, 팩·맥락 수는 대표 카드의 기존 상세에만 두고 네 결의 방향은 합산하지 않음
- 공유 후보: 비연애 공개 관계의 안전한 근거가 윤곽 이상인 결 전체
- 내 질문팩 관리: draft 이어하기, 완료 팩 공유와 과거 version 재현
...
9. `a`/`b`는 위치 marker로 표시하고 지인 집계에만 `trace/outline/clear`에 대응하는 넓은/중간/좁은 비통계적 band를 더한다. `contextual`은 중앙점 없는 양쪽 split, `unsettled`은 중앙점 없는 넓은 neutral 상태로 표시하며 API에 범위 좌표를 저장하지 않는다.
...
계정 통합 `/me`의 concept profile은 전체 공유 가능 후보를 stable rank로 보존한다. 공유 가능한 결이 3개 이상이면 주인이 `영역 · A—B` picker에서 대표 결 하나를 확인하고 `내 겹 공유하기`를 진행한다. 공유 묶음은 선택한 결을 첫 축에 고정한 뒤 기존 rank를 지키면서 가능한 한 서로 다른 영역의 결 2개를 더 고른다. 후보가 3개보다 적으면 concept 공유 CTA를 표시하지 않고 `시선 더 모으기`와 기존 팩 관리 행동을 유지한다.

`내 겹 공유하기`는 공개 프로필을 만들지 않는다. 주인이 공개하기로 정한 닉네임, `● 나 · ○ 지인`, 정확히 3개의 영역·결·A/B endpoint·고유 문항 수, settled인 내 위치와 공개 기준을 통과한 익명 지인 범위만 1080×1920 PNG에 담는다. 지인 결과가 contextual이면 중앙 marker 없이 양쪽 split으로 표시하고, 내 방향이 contextual·unsettled이거나 지인 근거가 윤곽 미만인 결은 후보에서 제외한다. 관찰문·질문·유형·점수·퍼센트·응답자 수·개별 답변·관계별 원자료·내부 ID·관리 URL·secret은 카드 데이터에서 제외한다. 서버는 query의 `share_concept`, source play, 3축 묶음을 현재 계정의 전체 공유 가능 후보로 다시 검증하며 위조·불일치는 404로 닫는다. 초대는 첫 축의 source pack을 재사용하고 수신자의 같은 팩 CTA는 기존 `same_pack_cta`를 유지한다. 파일 공유 미지원·취소·`NotAllowedError`를 포함한 실패에서는 생성된 링크를 보존하고 이미지 저장과 링크 복사를 각각 제공한다.
...
- 제품 전환과 ordered funnel의 canonical SSOT는 `public.analytics_events`와 `private.core_funnel_stage_counts`다. GA4 수치는 이 값을 대체하거나 보정하지 않는다.
- GA4는 현재 브라우저에서 명시적으로 허용한 traffic의 coarse route-class `page_view`와 운영 태그 건강 확인에만 사용한다. pending·denied에서는 Google script와 수집 요청을 만들지 않는다.
- 실제 play/public/response/Auth ID, query·fragment·referrer·campaign 원문, 닉네임·이메일·답변·관계·알게 된 시점, 내부 제품 event·user property·custom dimension은 GA4에 보내지 않는다.
- 분석 허용과 중단은 제품 사용을 막지 않으며 `/privacy`에서 현재 브라우저의 선택을 바꿀 수 있다.
...
- 관계와 해당 질문 표본, 친구 시선의 우세 선택과 셀프 선택 일치 여부를 결과로 먼저 표시
- 우세가 없는 동수는 해석하지 않고 `시선이 반으로 갈렸어요`로 표시
- 관계 이름과 팩 이름
- 응답자 개인 신원은 기본적으로 숨김
...
GA4 route-class page view는 consented traffic의 대략적인 화면 방문과 태그 상태만 확인하는 보조 지표다. 제품 퍼널 전환율이나 실험 판정에는 사용하지 않는다.
...
P0 최초 공식 팩과 타깃 관계는 **오래된 친구**로 시작했다. 단일 팩으로 서버 저장·공유·비교·프로필 루프를 완성했고, 현재 비공개 재미 검증에서는 5.2의 검수된 공식 팩 24개를 함께 활성화한다. public beta와 다음 공식 팩 추가는 24팩의 핵심 루프 지표를 확인한 뒤 별도로 결정한다.
