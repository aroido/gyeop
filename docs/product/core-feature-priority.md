...
인증된 owner의 `/me` 첫 정보 구조는 질문팩 목록이 아니라 owner가 직접 입력한 닉네임을 제목으로 한 계정 통합 프로필이다. 완료 play별 셀프 대표 카드와 이미 공개 가능한 관계·질문 집계를 한 사람의 프로필에 모으되, 각 play의 threshold 판정은 그대로 보존한다. 전체 수치는 완료 play별 submitted 공개 응답 수를 더한 `시선`이며 고유 사람 수로 표현하지 않는다. draft와 완료 play의 이어 답기·공유 관리는 프로필 보상 아래 `내 질문팩 관리`에 둔다.
...
7. 배포 호환용 top-level 카드 합계는 공개 가능한 관계의 공개 가능한 질문만 합산한다. 관계 또는 질문 표본이 1~2인 값은 다른 관계와 합쳐 3을 만들지 않고 top-level에서도 `0/null`로 남긴다.
8. 관계 선택기 최초 선택은 공개 가능한 관계를 registry 순서로 우선하고, 없으면 첫 수집 중 관계로 정한다. `known_since_code`는 저장만 유지하고 이 화면의 교차 집계·필터에 쓰지 않는다.
9. 계정 통합 프로필은 완료 play마다 위 규칙을 통과한 strict profile만 조합한다. 서로 다른 play의 관계 2+1·2+2나 질문 표본 1+2를 합쳐 공개 기준을 열지 않는다.
...
계정 통합 `/me` 상단 CTA는 실제 프로필 상태에 따라 하나만 표시한다. 완료 play에 `romantic`을 제외한 공개 가능 관계·질문 집계가 있으면 `내 겹 공유하기`로 해당 카드가 선택된 주인 전용 프로필에 진입한다. 완료 play는 있지만 공유 가능한 카드가 없으면 `시선 더 모으기`로 공유 관리에 진입하고, 완료 play가 없으면 `질문팩 시작하기`로 홈에 진입한다.

`내 겹 공유하기`는 공개 프로필을 만들지 않는다. 주인이 확인한 관계 하나와 질문 하나의 원본 문구·선택지·셀프 선택·A/B 수치만 1080×1920 PNG로 브라우저에서 만들고, `이 카드 공유하기` 한 번으로 필요한 public 초대 링크 생성과 가능한 즉시 이미지+링크 OS 공유까지 이어간다. 관계와 해당 질문 표본, 친구 시선의 우세 선택, 셀프 선택과의 일치 여부를 결과로 먼저 보여 주고 원본 질문과 A/B 분포를 보조로 둔다. 동수면 우세나 일치를 만들지 않고 `시선이 반으로 갈렸어요`로 표시한다. 관계와 질문 표본이 각각 3명 이상인 값만 사용하며 `romantic`, 1:1 응답, 닉네임, 내부 ID, 관리 URL과 secret은 카드 데이터에서 제외한다. 파일 공유 미지원·취소·실패 시 생성된 링크를 보존하고 이미지 저장과 링크 복사를 각각 제공한다.
