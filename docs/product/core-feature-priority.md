...
- private MVP 모집과 production beta의 이용 자격은 `대한민국에서 이용하는 만 19세 이상`이다. 주인과 방문자에 같은 기준을 적용하고 만 19세 미만용 보호자 동의 흐름은 만들지 않는다.
- 팩 선택 뒤 새 play 생성 전 자기확인을 받고, #16의 서버 생성 경계가 exact `eligibilityConfirmed: true`를 검증하기 전에는 play·cookie·analytics를 만들지 않는다. 생년월일·신분증·보호자 정보·IP geolocation은 수집하지 않는다.
- 현재 `비공개 재미 검증`에서 팩 주인은 이메일·전화번호·표시 이름·Auth 계정 없이 시작한다.
- 서버가 발급한 256-bit play 관리 secret은 `Secure`·`HttpOnly`·`SameSite=Lax` same-browser cookie에만 두고, DB에는 domain-separated hash만 저장한다. 같은 cookie의 play id+hash가 모두 맞을 때만 7일 inactivity window 동안 draft·완료 상태를 복구한다.
- cookie를 잃거나 만료·로그아웃하면 이메일·다른 기기·운영자 경로로 복구하거나 재발급하지 않는다. 새 play는 invalid cookie를 지운 뒤 사용자가 다시 명시적으로 시작한다.
- 이메일 매직 링크 계정 연결, cross-device 복구, 이메일 알림, owner 계정 삭제는 `production beta 재승인` 후보이며 비공개 재미 검증의 완료 조건이 아니다.
...
- 관계 선택과 새 response 생성 전 주인과 같은 만 19세·대한민국 자기확인을 받는다. 미해당이면 response·session cookie·assignment·event·link consume을 만들지 않고, 차단 사실을 팩 주인에게 알리지 않는다.
...
- 최소 이용 연령·미성년자 데이터 정책의 canonical SSOT는 `docs/product/age-and-minor-policy.md`다.
...
- 비공개 재미 검증은 Auth owner 계정을 만들지 않으므로 owner 계정 삭제 UI·worker를 완료 조건으로 두지 않는다.
- same-browser owner는 로그아웃으로 현재 play 관리 hash와 cookie를 즉시 폐기한다. 마지막 owner 활동+7일에 play와 연결 데이터를 만료시키고 24시간 안에 운영 DB에서 삭제한다.
...
- 신고된 미성년자 데이터는 안전한 capability로 대상을 찾은 `target_located_at`부터 live 72시간 안에 hard-delete하고 backup은 `hard_deleted_at`부터 30일 안에 만료한다. #7은 application backup 밖 최소 45일 HMAC 삭제 ledger와 격리 restore·재삭제·0건 검증을 더 엄격한 상한으로 집행한다.
...
- 주인과 방문자는 새 domain row 생성 전 `대한민국에서 이용하는 만 19세 이상`임을 직접 확인하며, 미해당 시 저장 없이 차단된다.
...
- 비공개 재미 검증의 주인은 same-browser 로그아웃으로 capability를 폐기할 수 있다. 일반 데이터는 `docs/product/data-retention-and-deletion-policy.md`, 미성년자 신고 데이터는 더 엄격한 `docs/product/age-and-minor-policy.md`를 따른다.
- production beta는 현행 한국 개인정보 법률 서면 검토, #16 UI/API 우회 차단, provider backup 30일 증빙, 2배 peak cleanup, live 72시간·ledger 45일/restore drill, 공개 privacy 문의 채널 없이는 열지 않는다.
