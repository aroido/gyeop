...
계정 통합 `/me`의 concept profile은 전체 공유 가능 후보를 stable rank로 보존한다. 첫 후보는 추천일 뿐 자동 확정하지 않으며, 주인이 picker에서 한 결을 확인해야 `한 장으로 나누기`를 진행한다. 후보가 없으면 concept 공유 CTA를 표시하지 않고 기존 팩 관리 행동을 유지한다.

`한 장으로 나누기`는 공개 프로필을 만들지 않는다. 주인이 선택한 안전한 결의 고정 관찰문·다음 질문·근거 요약과 그 결의 source pack 공개 초대만 1080×1920 PNG에 담는다. 서버는 query의 `share_concept`와 source play를 현재 계정의 공유 가능 후보로 다시 검증하며 위조·불일치는 404로 닫는다. `romantic`, 1:1, 닉네임, 내부 ID, 관계별 원자료, 점수·퍼센트, 관리 URL과 secret은 카드 데이터에서 제외한다. 파일 공유 미지원·취소·실패 시 생성된 링크를 보존하고 이미지 저장과 링크 복사를 각각 제공한다.
