...
인증된 owner의 `/me` 첫 정보 구조는 질문팩 목록이 아니라 owner가 직접 입력한 닉네임을 제목으로 한 계정 통합 프로필이다. `GYEOP_CONCEPT_PROFILE_ENABLED=true`에서는 완료한 concept v1 팩의 여러 장면을 8개 영역·32개 양방향 결로 연결해 3~5개의 대화 훅을 먼저 보여 준다. 결은 사람 전체를 규정하는 유형·진단·점수가 아니며, `반복해서 보이는 결`, `나와 주변 시선의 차이`, `상황에 따라 달라지는 결`처럼 실제 답변 근거에서만 만든다. feature가 꺼져 있으면 기존 관계 레이어 프로필을 그대로 유지한다.
...
- 대화 훅 3~5개: 고정 유형 대신 여러 팩·맥락에서 반복되거나 달라진 결
- 근거 상세: `왜 이렇게 보일까?`에서 팩 제목과 검수된 맥락만 표시
- 내가 보는 나와 주변 시선: 서로 합산하지 않은 별도 근거
- 부족 상태: 임계값을 넘기 전에는 방향 대신 `시선을 모으는 중 · n/3`
