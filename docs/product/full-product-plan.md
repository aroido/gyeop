...
- 팩 주인은 결과를 저장할 때 이메일 매직 링크로 계정을 연결한다.
...
- 운영자가 만들고 사람이 검수한 공식 팩 `오래 본 너의 시선`, `처음 만난 너의 시선`, `같이 일한 너의 시선`, `가까운 너의 시선`을 비공개 재미 검증에 제공한다.
- 팩 카드에서 추천 관계, 질문 수, 예상 시간, 분위기, 민감도, 1:1 추천 여부를 보여준다.
- 새 공식 팩 후보와 public beta 노출은 이 네 팩의 핵심 루프 지표를 확인한 뒤 순차적으로 검토한다.
...
- 인증된 주인은 계정 삭제를 요청할 수 있고, 연결된 live application 데이터는 확정된 보관 정책에 따라 제거한다.
...
- 인증된 주인의 계정 삭제와 확정된 보관·backup 완전 삭제 기준이 동작한다.
