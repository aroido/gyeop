...
초기 주 사용자는 Instagram, KakaoTalk, 문자 링크 공유에 익숙한 모바일 사용자다. 특정 연령을 확정하지 않으며 최소 이용 연령과 미성년자 정책은 출시 전에 별도 결정한다.
...
| 친구 응답 | 방문자가 주인을 떠올리며 제출한 Signature 1장과 균형 추출 2장 |
| 시선 | 셀프 응답과 친구 응답의 카드별 비교에서 생긴 한 사람의 관점 |
| 겹 프로필 | 여러 팩과 관계에서 받은 시선이 누적된 주인의 프로필 |
...
- 팩 주인은 결과를 저장할 때 소셜 로그인 또는 이메일 매직 링크로 계정을 연결한다.
- 구체적인 인증 제공자와 프레임워크는 구현 스펙에서 결정한다.

### 6.2 공식 팩 선택

- 운영자가 만든 공식 팩 4~6개로 시작한다.
- 팩 카드에서 추천 관계, 질문 수, 예상 시간, 분위기, 민감도, 1:1 추천 여부를 보여준다.
- P0 첫 타깃 관계는 하나로 좁힌다. 현재 추천은 `오래된 친구`이며 최종 확정이 필요하다.
...
P0 프로필은 완성형 SNS가 아니라 응답이 사라지지 않고 축적된다는 사실을 증명한다.
...
P0에서는 이메일 또는 웹 푸시 중 하나만 선택한다. 방문자에게 재촉 알림을 자동 발송하지 않는다.
...
- 전체 시선 요약
- 관계별 시선 레이어
- 셀프 인식과 타인 인식의 카드별 차이
- 표본이 충분한 관계끼리의 비교
- 시간 변화는 각 관계 레이어 안의 보조 탐색

같은 관계 응답자가 3명 이상일 때만 공개 관계별 집계를 만든다. 3명 미만이면 `아직 시선을 모으는 중`으로 표시한다.

### 7.4 SNS 공유 카드

- 서비스보다 사용자의 발견을 주어로 쓴다.
- 관계 이름과 팩 이름을 표시한다.
...
- 주인용 문장과 자동 변환된 방문자용 문장을 발행 전에 함께 검수한다.
...
| 관계별 집계 | 최소 표본 기준, 공개 상태, 삭제·철회 재계산 |
...
- 같은 관계 응답자 3명 미만에서는 공개 집계를 만들지 않는다.
...
- 첫 공식 팩의 단일 타깃 관계를 `오래된 친구`로 확정할지
- P0 공식 팩의 정확한 4~6개 구성
- 주인 계정 연결을 소셜 로그인과 이메일 매직 링크 중 무엇으로 시작할지
- P0 알림 채널을 이메일과 웹 푸시 중 무엇으로 할지
- 최소 이용 연령과 미성년자 데이터 정책
- 링크 만료, 재발급, 철회와 데이터 보관 기간
...
사용자 팩 제작, 팩 마켓, 점수·순위, 자유 텍스트, 피드·댓글·DM, 공개 사용자 검색, 광고·결제, AI 심리 진단.
