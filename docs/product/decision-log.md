...
- 결과: anticipation → tear → extract → settle 순서를 짧은 스크롤 안에서 겹쳐 재생하고, 카드는 0.96–1.02배 안에서만 움직인다. 약 220px runway, 85% 스냅, 역스크롤, 키보드 버튼, reduced-motion 즉시 진입, 질문 handoff와 자체 GYEOP 그래픽 계약은 그대로 유지한다.
