...
export function decodeAssignOptionalCardsOutcome(value) {
  if (value?.outcome === "assigned") {
    const outcome = stateOutcome(value, "assigned");
    if (
      outcome.response.status !== "submitted" ||
      outcome.response.assignments.filter(({ stage }) => stage === "optional")
        .length !== 2
    ) {
      invalid();
    }
  ...
  return simpleOutcome(value, ["not_submitted", "session_invalid"]);
}
...
export function decodeWithdrawResponseOutcome(value) {
  return simpleOutcome(value, ["withdrawn", "unavailable"]);
}
