import { OFFICIAL_PACK_HISTORY } from "../packs/official-pack-registry.mjs";
...
const PACK_TITLES = Object.freeze({
  "after-work": "퇴근 후 본캐",
  "algorithm-mirror": "알고리즘이 들킨 날",
  "camera-roll": "최근 사진 20장",
  "comment-section": "내적 댓글창",
  "compliment-receipt": "칭찬 영수증",
  coworker: "퇴근 전의 우리",
  "deadline-mode": "마감 전의 나",
  "decision-spiral": "결정은 내일의 나에게",
  "emoji-subtitles": "이모지 자막 켜기",
  "first-impression": "첫 장면, 네 버전",
  "friend-fusion": "우리 둘이 섞인다면",
  "group-chat-role": "단톡방의 나",
  "honest-self": "말 안 해도 알까?",
  "laugh-track": "웃음 버튼의 위치",
  "old-friend": "우리는 아직도 통하는 편",
  "reply-temperature": "읽고도 생각 중",
  "room-temperature": "방 온도와 내 기분",
  "small-luxury": "작은 사치 연구소",
  "snack-personality": "간식 취향 보고서",
  "social-battery": "사람들 사이 배터리",
  "spontaneous-plan": "갑자기, 지금, 같이?",
  "tiny-routine": "혼자만의 루틴",
  "trip-chemistry": "여행 가방의 철학",
  "weekend-escape": "주말 사용 설명서",
});
const PACKS = Object.freeze(
  Object.fromEntries(
    OFFICIAL_PACK_HISTORY.map(({ slug, version }) => [
      `${slug}\0${version}`,
      Object.freeze({ packVersion: version, packTitle: PACK_TITLES[slug] }),
    ]),
  ),
);
...
function decodePackMetadata(value) {
  const expected =
    ...
    Object.prototype.hasOwnProperty.call(
      PACKS,
      `${value.packSlug}\0${value.packVersion}`,
    )
      ? PACKS[`${value.packSlug}\0${value.packVersion}`]
      : undefined;
  if (
    !expected ||
