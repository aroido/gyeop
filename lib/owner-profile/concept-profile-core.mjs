...
import { isProfileShareRelationship } from "./profile-share-card-core.mjs";
...
function candidateForConcept(concept, sources) {
  ...
  return {
    ...
    shareEvidence,
    shareEligible: shareEvidence.status === "available",
...
export function buildConceptProfile({ pairs }) {
  ...
  if (pairs.length === 0)
    return freeze({ modelVersion: 1, hooks: [], shareOptions: [] });
  ...
  for (const candidate of candidates) {
    ...
    if (
      ...
      (candidate.kind === "repeated" || candidate.kind === "emerging")
    ) {
  ...
  const shareOptions = shareable.slice(0, 32).map((candidate) => ({
    conceptId: candidate.conceptId,
    safeCopy: candidate.shareEvidence.observation,
    safeQuestion: candidate.shareEvidence.question,
    shareEvidence: candidate.shareEvidence,
  ...
  return decodeConceptProfile(
    ...
    shareable.slice(0, 32).map(({ conceptId }) => conceptId),
  );
}
...
export function decodeConceptProfile(value, expectedShareOptionOrder = null) {
  if (
    !exactKeys(value, ["hooks", "modelVersion", "shareOptions"]) ||
    value.modelVersion !== 1 ||
    !Array.isArray(value.hooks) ||
    !Array.isArray(value.shareOptions) ||
    ![0, 3, 4, 5].includes(value.hooks.length) ||
    value.shareOptions.length > 32
  ) {
    invalid();
  }
  if (
    expectedShareOptionOrder !== null &&
    (!Array.isArray(expectedShareOptionOrder) ||
      !deepEqual(
        value.shareOptions.map(({ conceptId }) => conceptId),
        expectedShareOptionOrder,
      ))
  ) {
    invalid();
  }
  ...
  for (const [index, hook] of value.hooks.entries()) {
    ...
    if (
      hook.shareEligible !== shareAvailable ||
      shareAvailable !== shareSourceEligible ||
  ...
  for (const option of value.shareOptions) {
    if (
      ...
      !boundedText(option.safeCopy, CONCEPT_COPY_LIMITS.observation) ||
      !boundedText(option.safeQuestion, CONCEPT_COPY_LIMITS.question) ||
      option.safeCopy !== option.shareEvidence.observation ||
      option.safeQuestion !== option.shareEvidence.question ||
      !isOwnerPlayId(option.sourcePlayId)
    ) {
      invalid();
    }
    const concept = conceptById(option.conceptId);
    validateAvailableShareEvidence(option.shareEvidence, concept, packTitles);
    const selected = hookByConcept.get(option.conceptId);
    if (selected) {
      if (
        !selected.hook.shareEligible ||
        !deepEqual(option.shareEvidence, selected.hook.shareEvidence) ||
        option.safeCopy !== selected.hook.shareEvidence.observation ||
        option.safeQuestion !== selected.hook.shareEvidence.question ||
  ...
  for (const hook of value.hooks) {
    if (hook.shareEligible && !optionIds.has(hook.conceptId)) invalid();
  }
  return freeze(value);
}
