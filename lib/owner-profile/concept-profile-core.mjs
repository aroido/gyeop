...
const CONCEPT_COMPATIBILITY_FIELDS = Object.freeze([
  "id",
  "position",
  "ownerPrompt",
  "visitorPrompt",
...
function indexUnique(values, keyFor) {
  const index = new Map();
...
function adaptHistoricalManifest(manifest, currentManifest) {
  if (
    manifest.slug !== currentManifest.slug ||
    manifest.cards.length !== 10 ||
    currentManifest.cards.length !== 10
  ) {
    invalid();
  }
  return {
    ...
    cards: manifest.cards.map((card, index) => {
      const currentCard = currentManifest.cards[index];
      if (
        !isConceptContextV1(currentCard.conceptContext) ||
        !Array.isArray(currentCard.conceptSignals)
      ) {
        invalid();
      }
      const compatible = CONCEPT_COMPATIBILITY_FIELDS.every(
        (field) => card[field] === currentCard[field],
      );
      return {
        ...card,
        conceptContext: currentCard.conceptContext,
        conceptSignals: compatible ? currentCard.conceptSignals : [],
      };
    }),
  };
}
...
export function selectConceptProfileSourcePairs({
  ...
  if (
    ...
    !Array.isArray(currentManifests)
  ) {
    invalid();
  }
  ...
  const manifestByVersion = indexUnique(
    manifests,
    (manifest) => `${manifest.slug}\0${manifest.version}`,
  );
  const currentManifestBySlug = indexUnique(
    currentManifests.filter((manifest) => manifest.conceptVersion === 1),
  ...
  for (const summary of plays) {
    ...
    const historicalManifest = manifestByVersion.get(
      `${summary.packSlug}\0${summary.packVersion}`,
    );
    const currentManifest = currentManifestBySlug.get(summary.packSlug);
    if (!profile || !historicalManifest || !currentManifest) continue;
    const manifest =
      historicalManifest.version === currentManifest.version
        ? currentManifest
        : adaptHistoricalManifest(historicalManifest, currentManifest);
    if (
...
function collectSources(pairs) {
  ...
  for (const pair of pairs) {
    ...
    for (const card of pair.manifest.cards) {
      if (
        !isConceptContextV1(card.conceptContext) ||
...
function candidateForConcept(concept, sources) {
  ...
  return {
    ...
    shareEvidence,
    shareEligible: directionAB(self),
...
function shareAxis(candidate) {
  ...
  const source = candidate.shareSafeOthers;
  let others;
  if (source.status === "locked") {
    others = { status: "locked", sightCount: source.sightCount };
  } else if (source.direction === "contextual") {
    others = {
      status: "available",
      source: "shareSafeOthers",
      direction: "contextual",
  ...
  } else if (source.direction === "unsettled") {
    others = {
      status: "available",
      source: "shareSafeOthers",
      direction: "unsettled",
    ...
    const position =
      "position" in source
        ? source.position
        : positionForDirectionScore(source.directionScore);
    others = {
      status: "available",
      source: "shareSafeOthers",
      direction: source.direction,
  ...
  return decodeConceptProfileShareAxis({
    ...
    selfPosition,
    others,
    cardCount: candidate.self.evidence.cardCount,
  });
}
  ...
  const shareOptions =
    shareable.length < 3
      ...
      : shareable.map((candidate) => ({
          ...
          sourcePlayId: candidate.profileSourcePlayId,
...
export function decodeConceptProfile(value, expectedShareOptions = null) {
  ...
  for (const [index, hook] of value.hooks.entries()) {
    ...
    const shareSourceEligible = directionAB(hook.self);
    if (
  ...
  for (const option of value.shareOptions) {
    ...
    if (selected) {
      if (
        ...
        option.sourcePlayId !== selected.hook.profileSourcePlayId ||
