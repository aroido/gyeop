...
const LOCKED_OTHERS_KEYS = Object.freeze(["sightCount", "status"]);
const UNSETTLED_OTHERS_KEYS = CONTEXTUAL_OTHERS_KEYS;
...
export function decodeConceptProfileShareAxis(value) {
  ...
  const valid =
    others?.status === "locked"
      ? exactKeys(others, LOCKED_OTHERS_KEYS) &&
        Number.isSafeInteger(others.sightCount) &&
        others.sightCount >= 0 &&
        others.sightCount <= 2
      : others?.direction === "contextual"
        ? exactKeys(others, CONTEXTUAL_OTHERS_KEYS) &&
          ...
          others.range === "split"
        : others?.direction === "unsettled"
          ? exactKeys(others, UNSETTLED_OTHERS_KEYS) &&
            others.status === "available" &&
            others.source === "shareSafeOthers" &&
            ["trace", "outline", "clear"].includes(others.stage) &&
            others.range === "neutral"
