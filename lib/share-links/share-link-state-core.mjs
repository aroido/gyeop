...
export function decodeInvitePreviewOutcome(value) {
  if (hasExactKeys(value, ["outcome"]) && value.outcome === "unavailable") {
  ...
  if (
    ...
    typeof value.previewNickname !== "string" ||
    value.previewNickname.normalize("NFKC") !== value.previewNickname ||
    [...value.previewNickname].length < 2 ||
    [...value.previewNickname].length > 12 ||
    !/^[가-힣A-Za-z0-9]+(?: [가-힣A-Za-z0-9]+)*$/u.test(
      value.previewNickname,
    ) ||
    (value.kind !== "public" && value.kind !== "one_to_one") ||
    typeof value.packSlug !== "string" ||
    !LOWER_KEBAB.test(value.packSlug) ||
    typeof value.packVersion !== "string" ||
    !LOWER_KEBAB.test(value.packVersion) ||
    typeof value.packTitle !== "string" ||
    value.packTitle.length < 1 ||
    value.packTitle.length > 120 ||
    !["low", "medium", "high"].includes(value.sensitivity)
  ) {
    invalid();
  }
