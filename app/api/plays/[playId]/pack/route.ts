import {
  ...
  readOwnerPlayPackResponse,
} from "../../../../../lib/http/owner-play.ts";
import { readAuthenticatedOwnerPlayPackResponse } from "../../../../../lib/http/auth-owner.ts";
      ...
      runRateLimitedDomain(
        ...
        async () => {
          ...
          if (cookie.outcome === "absent") {
            return readAuthenticatedOwnerPlayPackResponse({ playId });
          }
          if (cookie.outcome === "malformed") {
            return ownerNotFoundResponse(true);
          }
          return readOwnerPlayPackResponse({ cookie, playId, signal });
        },
      ),
  );
}
